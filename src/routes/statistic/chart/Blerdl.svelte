<script lang="ts">
  ...
  import { getLabelSettings } from "./labelsSettingsdrop";
  import { fetchStatisticChartData } from "./chartdata";
  ...
  function updateChart() {
    const bandData = chartdata.dl_bler_by_region.band[selectedBand];
    if (!bandData) return;
    const { region, avg } = bandData;

    const backgroundColors = avg.map(value => value >= 1 ? 'rgba(245, 40, 45, 0.8)' : 'rgba(30, 166, 236, 0.8)')
    const borderColors = avg.map(value => value >= 1 ? 'rgba(245, 40, 145, 0.8)' : 'rgba(61, 233, 177, 0.8)');

    const minYValue = Math.min(...avg) - 5;
    const maxYValue = Math.min(...avg) + 0.5
    ...
    if (Availability) {
        ...
        Availability.options.scales.y.max = maxYValue;
        Availability.update();
    }
  }
