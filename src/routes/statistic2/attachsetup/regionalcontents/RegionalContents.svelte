<script lang="ts">
  ...
  import { trendDataStore, fetchTrendData } from "./FetchattachData";
	import { Tooltip } from "flowbite-svelte";


  import RegionalattachAttempt from "../chart/RegionalAttachAttempt.svelte";
  import RegionalattachTrendbyBand from "../chart/RegionalAttachTrendbyBand.svelte";
  import RegionalattachFailCluster from "../chart/RegionalattachFailCluster.svelte";
  import RegionalattachFailSiteCell from "../chart/RegionalattachFailSiteCell.svelte";
  ...
  let field: string = 'attach_setup_success_rate';
  let AttachRegionalData: any = null;
  let band: string;

  $: AttachRegionalData = $trendDataStore
  $: $trendDataStore, AttachRegionalData = $trendDataStore
  $: if (region) {
    (region);
  }
  ...
  const AttachAttemptCountChartKeyMap: { [key: string]: string} = {
    'attachnominatebyband': 'Total Attempt',
    'connestabbyband': 'ConnEstab',
    's1connestabbyband': 'S1ConnEstab',
    'establnitbyband': 'EstabInit'
  };
  const chartKeys = Object.keys(AttachAttemptCountChartKeyMap);
  ...
  $: selectedNominateData = getSelectedData(AttachRegionalData, selectedChartKey)
  ...
  $: SelectedNominateOverallData = getSelectedNominateOverallData(AttachRegionalData, selectedChartKey)

  $: FailClusterCount = 'fail_cluster_count_byband';
</script>

<div class="grid grid-cols-8 gap-4 w-full mb-4">
  <!-- REGIONAL ATTACH FAIL & ATTEMPT COUNT -->
  <div class="flex flex-col col-span-2 bg-slate-900/40 rounded-md p-4">
    <div class="flex items-center gap-1">
      ...
      <h1 class="text-xs text-slate-500">{region.toLocaleUpperCase()} ATTACH ATTEMPT COUNT</h1>
    </div>

    {#if AttachRegionalData}
      <div class="flex flex-col items-center justify-center h-full">
        <div class="flex flex-col">
          <h1 class="text-2xl text-white font-bold">
            {((AttachRegionalData.attachnominatebyband.overall.deNomnateTotal) - 
            (AttachRegionalData.attachnominatebyband.overall.NominateTotal)).toLocaleString()}
          </h1>
          <h1 class="text-xs text-slate-500">
            {(((AttachRegionalData.attachnominatebyband.overall.deNomnateTotal) - 
              (AttachRegionalData.attachnominatebyband.overall.NominateTotal)) / 7).toLocaleString()} (Per Day)
          </h1>
        </div>
      </div>
      ...
      <div class="flex items-center gap-1">
        <div class="rounded-sm px-1 py-[1px] bg-green-500 text-xs">
          {
            ((((AttachRegionalData.attachnominatebyband.overall.NominateTotal) - 
              (AttachRegionalData.attachnominatebyband.overall.deNomnateTotal)) / 
              ((data.AttachStatisticData.Attach.attachnominatebyband.overall.NominateTotal) - 
              (data.AttachStatisticData.Attach.attachnominatebyband.overall.deNomnateTotal))) * 
              100).toFixed(1)
          }%
        </div>
        <h1 class="text-xs text-slate-500">Share of the national attach fail count</h1>
      </div>
    {/if}
  </div>

  <!-- REGIONAL ATTACH COUNT BY BAND -->
  <div class="flex flex-col col-span-2 bg-slate-900/40 rounded-md p-4 gap-4">
    <div class="relative flex items-start gap-1">
      <Icon icon="fluent:calendar-week-numbers-24-regular" class="text-lg text-slate-500" />
      <div class="flex flex-col">
        <h1 class="text-xs text-slate-500">ATTACH ATTEMPT BY BAND</h1>
      </div>
      ...
      {#if showDropdown}
        <div class="z-50 absolute -top-5 right-5 flex flex-col bg-slate-700/90 shadow-md rounded-md mt-2">
          ...
          on:click={() => {selectedChartKey = key; showDropdown = false}}>
            {AttachAttemptCountChartKeyMap[key]}
          </button>
          {/each}
        </div>
      {/if}
    ...
    <div class="flex items-center gap-4">
      <div class="relative flex items-center justify-center">
        <div class="">
          <RegionalattachAttempt {AttachRegionalData} chartKey={selectedChartKey} />
        </div>
        <div class="absolute flex flex-col items-center justify-center gap-1">
          ...
          <h1 class="text-xs text-white">{AttachAttemptCountChartKeyMap[selectedChartKey]}</h1>
        </div>
      </div>
  ...
  <!-- REGIONAL ATTACH TREND BY BAND -->
  <div class="flex flex-col col-span-4 bg-slate-900/40 rounded-md p-4">
    <div class="flex items-center gap-1 mb-4">
      <Icon icon="fluent:calendar-week-numbers-24-regular" class="text-lg text-slate-500" />
      <h1 class="text-xs text-slate-500">{region.toLocaleUpperCase()} ATTACH TREND BY BAND</h1>
    </div>
    <div class="">
      <RegionalattachTrendbyBand {AttachRegionalData} />
    </div>
  </div>

  <!-- REGIONAL ATTACH FAIL CLUSTER -->
  <div class="flex flex-col col-span-2 bg-slate-900/40 rounded-md p-4">
    <div class="flex items-center gap-1 mb-4">
      <Icon icon="fluent:calendar-week-numbers-24-regular" class="text-lg text-slate-500" />
      <h1 class="text-xs text-slate-500">{region.toLocaleUpperCase()} ATTACH FAIL CLUSTER</h1>
    </div>
    <div class="flex items-center justify-around">
      <div class="relative flex items-center justify-center">
        <RegionalattachFailCluster {AttachRegionalData} />
        <div class="absolute flex flex-col items-center justify-center">
          <h1 class="text-xs text-slate-500">Fail Cluster</h1>
          <h1 class="text-2xl text-white font-bold">
            {#if AttachRegionalData}{AttachRegionalData.fail_cluster_count_all}{/if}
          </h1>
        </div>
      </div>
      ...
      <div class="flex flex-col gap-2">
        <div class="flex flex-col">
          <div class="flex flex-col gap-4">
            {#if AttachRegionalData && AttachRegionalData[FailClusterCount]}
              {#each AttachRegionalData[FailClusterCount].band as bands, index}
                <div class="flex items-center">
                  <button class="flex items-center justify-between p-2 gap-2 text-xs text-white w-full rounded-md 
                    ...
                    <h1 class="text-xs text-black w-4 h-4 rounded-full bg-rose-300">
                      {AttachRegionalData[FailClusterCount].count[index]}
                    </h1>
                  </button>
                </div>
  ...
  <!-- REGIONAL ATTACH FAIL SITE & CELL -->
  <div class="flex flex-col col-span-3 bg-slate-900/40 rounded-md p-4">
    <div class="flex items-center gap-1 mb-4">
      <Icon icon="fluent:calendar-week-numbers-24-regular" class="text-lg text-slate-500" />
      <h1 class="text-xs text-slate-500">{region.toLocaleUpperCase()} ATTACH FAIL SITE & CELL</h1>
    </div>
    <div class="flex items-center justify-around">
      <div class="relative flex items-center justify-center">
        <RegionalattachFailSiteCell {AttachRegionalData} />
        <div class="absolute flex flex-col items-center justify-center">
          <h1 class="text-xs text-slate-500">Site / Cell Fail</h1>
          <h1 class="text-lg text-white font-bold">
            {#if AttachRegionalData}
              {AttachRegionalData.fail_site_count_all}/{AttachRegionalData.fail_cell_count_all}
            {/if}
          </h1>
        </div>
      </div>
      <div class="grid grid-cols-3 gap-2">
        ...
        <div class="flex items-center justify-center w-full px-1 py-[1px] rounded-sm bg-rose-400">
          <h1 class="text-xs text-white">
            {#if AttachRegionalData}
              {#if AttachRegionalData.fail_site_count_byband.band.includes('2.3GHz')}
                {AttachRegionalData.fail_site_count_byband.count[AttachRegionalData.fail_site_count_byband.band.indexOf('2.3GHz')]}
              {:else}
              -
              {/if}
            {/if}
        ...
        <div class="flex items-center justify-center w-full px-1 py-[1px] rounded-sm bg-rose-400">
          <h1 class="text-xs text-white">
            {#if AttachRegionalData}
              {#if AttachRegionalData.fail_cell_count_byband.band.includes('2.3GHz')}
                {AttachRegionalData.fail_cell_count_byband.count[AttachRegionalData.fail_site_count_byband.band.indexOf('2.3GHz')]}
              {:else}
              -
              {/if}
            {/if}
        ...
        <div class="flex items-center justify-center w-full px-1 py-[1px] rounded-sm bg-rose-400">
          <h1 class="text-xs text-white">
            {#if AttachRegionalData}
              {#if AttachRegionalData.fail_site_count_byband.band.includes('2.6GHz')}
                {AttachRegionalData.fail_site_count_byband.count[AttachRegionalData.fail_site_count_byband.band.indexOf('2.6GHz')]}
              {:else}
              -
              {/if}
            {/if}
        ...
        <div class="flex items-center justify-center w-full px-1 py-[1px] rounded-sm bg-rose-400">
          <h1 class="text-xs text-white">
            {#if AttachRegionalData}
              {#if AttachRegionalData.fail_cell_count_byband.band.includes('2.6GHz')}
                {AttachRegionalData.fail_cell_count_byband.count[AttachRegionalData.fail_site_count_byband.band.indexOf('2.6GHz')]}
              {:else}
              -
              {/if}
            {/if}
        ...
        <div class="flex items-center justify-center w-full px-1 py-[1px] rounded-sm bg-rose-400">
          <h1 class="text-xs text-white">
            {#if AttachRegionalData}
              {#if AttachRegionalData.fail_site_count_byband.band.includes('800M')}
                {AttachRegionalData.fail_site_count_byband.count[AttachRegionalData.fail_site_count_byband.band.indexOf('800M')]}
              {:else}
              -
              {/if}
            {/if}
        ...
        <div class="flex items-center justify-center w-full px-1 py-[1px] rounded-sm bg-rose-400">
          <h1 class="text-xs text-white">
            {#if AttachRegionalData}
              {#if AttachRegionalData.fail_cell_count_byband.band.includes('800M')}
                {AttachRegionalData.fail_cell_count_byband.count[AttachRegionalData.fail_site_count_byband.band.indexOf('800M')]}
              {:else}
              -
              {/if}
            {/if}
  ...
  <!-- REGIONAL ATTACH MOST FAIL CLUSTER AND CELL LIST -->
  <div class="flex flex-col col-span-3 bg-slate-900/40 rounded-md">
    ...
    <div class="flex items-center rounded-md pb-2">
      <table class="w-full">
        ...
        <tbody>
          {#if AttachRegionalData && AttachRegionalData[topImpactClusterListKey]}
            {#each Object.entries(AttachRegionalData[topImpactClusterListKey]) as [band, data]}
              {#each data.cluster as cluster, index}
                <tr class="text-xs text-slate-300 py-2">
                  <td class="py-1 px-4">{cluster}</td>
                  <td class="py-1 px-4">{(data.attachfailtotal[index]).toLocaleString()}</td>
                  <td class="py-1 px-4">{((data.attachfailtotal[index] / AttachRegionalData.attachfallbybandall.by_band.failcount[2]) * 100).toFixed(2)}%</td>
  ...
  <div class="flex flex-col col-span-3 bg-slate-900/40 rounded-md">
    <div class="flex items-center p-4 gap-1">
      ...
      <h1 class="text-xs text-slate-500">ATTACH CLUSTER</h1>
    </div>
  ...
  <div class="flex flex-col col-span-5 bg-slate-900/40 rounded-md">
    <div class="flex items-center p-4 gap-1">
      ...
      <h1 class="text-xs text-slate-500">ATTACH CELL</h1>
    </div>
