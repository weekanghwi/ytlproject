<script lang="ts">
  ...
  let AttachOverallTrendChartbyBand: Chart;
  ...
  function updateChart() {
    const TrendData = OverallData.attach_trend_23;
    if (!TrendData) return;
    ...
    const datasets = [
      OverallData.attach_trend_23,
      OverallData.attach_trend_26,
      OverallData.attach_trend_800,
    ].filter(Boolean);
    ...
    if (AttachOverallTrendChartbyBand) {
      AttachOverallTrendChartbyBand.options.scales.y = {
        display: true,
        beginAtZero: false,
        min: yAxisMin,
        // max: yAxisMax,
      ...
      AttachOverallTrendChartbyBand.data.labels = weeknums;
      AttachOverallTrendChartbyBand.data.datasets[0] = {
        label: "2.3G Trend",
        data: values,
      ...
      const secondLineData = OverallData.attach_trend_26; // 
      const secondDataset = {
        label: "2.6G Trend",
      ...
      const thirdLineData = OverallData.attach_trend_800;
      const thirdDataset = {
        label: "800M Trend",
      ...
      // 데이터셋 배열 업데이트
      AttachOverallTrendChartbyBand.data.datasets = [AttachOverallTrendChartbyBand.data.datasets[0], secondDataset, thirdDataset];

      // 차트 업데이트
      AttachOverallTrendChartbyBand.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !AttachOverallTrendChartbyBand) {
      Chart.register(ChartDataLabels);

      AttachOverallTrendChartbyBand = new Chart(ctx, {
