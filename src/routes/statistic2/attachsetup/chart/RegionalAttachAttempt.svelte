<script lang="ts">
  ...
  export let AttachRegionalData: any = null;
  export let chartKey: string;

  let canvas: HTMLCanvasElement;
  let AttachAttemptChart: Chart;
  ...
  function updateChart(chartKey: string) {
    const RegionalData = processAPIData(AttachRegionalData, chartKey)
    ...
    if (AttachAttemptChart) {
      AttachAttemptChart.data.labels = band;
      AttachAttemptChart.data.datasets[0].data = ratio;
      AttachAttemptChart.data.datasets[0].backgroundColor = backgroundColors;
      AttachAttemptChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !AttachAttemptChart) {
      Chart.register(ChartDataLabels);

      AttachAttemptChart = new Chart(ctx, {
