<script lang="ts">
  ...
  let AttachOverallTrendChart: Chart;
  ...
  function updateChart() {
    const TrendData = OverallData.attach_trend_all;
    if (!TrendData) return;
    const { weeknums, values } = TrendData;

    if (AttachOverallTrendChart) {
      AttachOverallTrendChart.data.labels = weeknums;
      AttachOverallTrendChart.data.datasets[0].data = values;
      AttachOverallTrendChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !AttachOverallTrendChart) {
      Chart.register(ChartDataLabels);

      AttachOverallTrendChart = new Chart(ctx, {
