<script lang="ts">
  ...
  let AttachOverallChartByBand: Chart;
  ...
  function updateChart() {
    const OverallChartData = OverallData.attachnominatebyband.by_band;
    ...
    if (AttachOverallChartByBand) {
      AttachOverallChartByBand.data.labels = band;
      AttachOverallChartByBand.data.datasets[0].data = ratio;
      AttachOverallChartByBand.data.datasets[0].backgroundColor = backgroundColors;
      AttachOverallChartByBand.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !AttachOverallChartByBand) {
      Chart.register(ChartDataLabels);

      AttachOverallChartByBand = new Chart(ctx, {
