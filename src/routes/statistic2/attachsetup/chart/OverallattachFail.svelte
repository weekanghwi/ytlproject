<script lang="ts">
  ...
  function updateChart() {
    const OverallTrendData = OverallData.attachfallbybandall.by_band;
    if (!OverallTrendData) return;
    const {band, failcount} = OverallTrendData
    ...
    if (AttachFailChart) {
      AttachFailChart.data.labels = band;
      AttachFailChart.data.datasets[0].data = failcount;
      AttachFailChart.data.datasets[0].backgroundColor = backgroundColors;
      AttachFailChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !AttachFailChart) {
      Chart.register(ChartDataLabels);

      AttachFailChart = new Chart(ctx, {
