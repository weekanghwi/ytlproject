<script lang="ts">
  ...
  export let AttachRegionalData: any = null; // 부모 컴포넌트에서 전달받은 데이터

  let canvas: HTMLCanvasElement;
  let AttachRegionalTrendChartbyBand: Chart;
  ...
  function updateChart() {
    const RegionalTrendData = AttachRegionalData.regional_attach_trend_23;
    ...
    const datasets = [
      AttachRegionalData.regional_attach_trend_23,
      AttachRegionalData.regional_attach_trend_26,
      AttachRegionalData.regional_attach_trend_800,
    ].filter(Boolean);
    ...
    if (AttachRegionalTrendChartbyBand) {
      AttachRegionalTrendChartbyBand.options.scales.y = {
        display: true,
        beginAtZero: false,
        min: yAxisMin,
        // max: yAxisMax,
      ...
      AttachRegionalTrendChartbyBand.data.labels = weeknums;
      AttachRegionalTrendChartbyBand.data.datasets[0] = {
        label: "2.3G Trend",
        data: values,
      ...
      const secondLineData = AttachRegionalData.regional_attach_trend_26; // 
      const secondDataset = {
        label: "2.6G Trend",
      ...
      const thirdLineData = AttachRegionalData.regional_attach_trend_800;
      const thirdDataset = {
        label: "800M Trend",
      ...
      // 데이터셋 배열 업데이트
      AttachRegionalTrendChartbyBand.data.datasets = [AttachRegionalTrendChartbyBand.data.datasets[0], secondDataset, thirdDataset];

      // 차트 업데이트
      AttachRegionalTrendChartbyBand.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !AttachRegionalTrendChartbyBand) {
      Chart.register(ChartDataLabels);

      AttachRegionalTrendChartbyBand = new Chart(ctx, {
