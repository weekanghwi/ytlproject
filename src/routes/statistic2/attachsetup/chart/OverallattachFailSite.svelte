<script lang="ts">
  ...
  function updateChart() {
    ...
    if (AttachFailSiteChart) {
      AttachFailSiteChart.data.labels = band;
      AttachFailSiteChart.data.datasets[0].data = count;
      AttachFailSiteChart.data.datasets[0].backgroundColor = backgroundColors;
      AttachFailSiteChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !AttachFailSiteChart) {
      Chart.register(ChartDataLabels);

      AttachFailSiteChart = new Chart(ctx, {
