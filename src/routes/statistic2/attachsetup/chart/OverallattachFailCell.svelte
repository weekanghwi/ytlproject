<script lang="ts">
  ...
  function updateChart() {
    ...
    if (AttachFailCellChart) {
      AttachFailCellChart.data.labels = band;
      AttachFailCellChart.data.datasets[0].data = count;
      AttachFailCellChart.data.datasets[0].backgroundColor = backgroundColors;
      AttachFailCellChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !AttachFailCellChart) {
      Chart.register(ChartDataLabels);

      AttachFailCellChart = new Chart(ctx, {
