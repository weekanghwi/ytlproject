<script lang="ts">
  ...
  let AttachTypeChartByBand: Chart;
  ...
  function updateChart(chartKey: string) {
    ...
    if (AttachTypeChartByBand) {
      AttachTypeChartByBand.data.labels = band;
      AttachTypeChartByBand.data.datasets[0].data = ratio;
      AttachTypeChartByBand.data.datasets[0].backgroundColor = backgroundColors;
      AttachTypeChartByBand.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !AttachTypeChartByBand) {
      Chart.register(ChartDataLabels);

      AttachTypeChartByBand = new Chart(ctx, {
