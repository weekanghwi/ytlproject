<script lang="ts">
  ...
  let InterFreqHOSiteChart: Chart;
  ...
  function updateChart() {
    ...
    if (InterFreqHOSiteChart) {
      InterFreqHOSiteChart.data.labels = band;
      InterFreqHOSiteChart.data.datasets[0].data = count;
      InterFreqHOSiteChart.data.datasets[0].backgroundColor = backgroundColors;
      InterFreqHOSiteChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !InterFreqHOSiteChart) {
      Chart.register(ChartDataLabels);

      InterFreqHOSiteChart = new Chart(ctx, {
