<script lang="ts">
  ...
  let InterFreqHOOverallTrendChartbyBand: Chart;
  ...
  function updateChart() {
    const TrendData = OverallData.interfreqho_trend_23;
    if (!TrendData) return;
    ...
    const datasets = [
      OverallData.interfreqho_trend_23,
      OverallData.interfreqho_trend_26,
      OverallData.interfreqho_trend_800,
    ].filter(Boolean);
    ...
    if (InterFreqHOOverallTrendChartbyBand) {
      InterFreqHOOverallTrendChartbyBand.options.scales.y = {
        display: true,
        beginAtZero: false,
        min: yAxisMin,
        // max: yAxisMax,
      ...
      InterFreqHOOverallTrendChartbyBand.data.labels = weeknums;
      InterFreqHOOverallTrendChartbyBand.data.datasets[0] = {
        label: "2.3G Trend",
        data: values,
      ...
      const secondLineData = OverallData.interfreqho_trend_26; // 
      const secondDataset = {
        label: "2.6G Trend",
      ...
      const thirdLineData = OverallData.interfreqho_trend_800;
      const thirdDataset = {
        label: "800M Trend",
      ...
      // 데이터셋 배열 업데이트
      InterFreqHOOverallTrendChartbyBand.data.datasets = [InterFreqHOOverallTrendChartbyBand.data.datasets[0], secondDataset, thirdDataset];

      // 차트 업데이트
      InterFreqHOOverallTrendChartbyBand.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !InterFreqHOOverallTrendChartbyBand) {
      Chart.register(ChartDataLabels);

      InterFreqHOOverallTrendChartbyBand = new Chart(ctx, {
