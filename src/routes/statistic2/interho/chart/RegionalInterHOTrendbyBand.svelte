<script lang="ts">
  ...
  export let InterFreqHORegionalData: any = null; // 부모 컴포넌트에서 전달받은 데이터

  let canvas: HTMLCanvasElement;
  let InterHORegionalTrendChartbyBand: Chart;
  ...
  function updateChart() {
    const RegionalTrendData = InterFreqHORegionalData.regional_interfreqho_trend_23;
    ...
    const datasets = [
      InterFreqHORegionalData.regional_interfreqho_trend_23,
      InterFreqHORegionalData.regional_interfreqho_trend_26,
      InterFreqHORegionalData.regional_interfreqho_trend_800,
    ].filter(Boolean);
    ...
    if (InterHORegionalTrendChartbyBand) {
      InterHORegionalTrendChartbyBand.options.scales.y = {
        display: true,
        beginAtZero: false,
        min: yAxisMin,
        // max: yAxisMax,
      ...
      InterHORegionalTrendChartbyBand.data.labels = weeknums;
      InterHORegionalTrendChartbyBand.data.datasets[0] = {
        label: "2.3G Trend",
        data: values,
      ...
      const secondLineData = InterFreqHORegionalData.regional_interfreqho_trend_26; // 
      const secondDataset = {
        label: "2.6G Trend",
      ...
      const thirdLineData = InterFreqHORegionalData.regional_interfreqho_trend_800;
      const thirdDataset = {
        label: "800M Trend",
      ...
      // 데이터셋 배열 업데이트
      InterHORegionalTrendChartbyBand.data.datasets = [InterHORegionalTrendChartbyBand.data.datasets[0], secondDataset, thirdDataset];

      // 차트 업데이트
      InterHORegionalTrendChartbyBand.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !InterHORegionalTrendChartbyBand) {
      Chart.register(ChartDataLabels);

      InterHORegionalTrendChartbyBand = new Chart(ctx, {
