<script lang="ts">
  ...
  export let InterFreqHORegionalData: any = null;
  export let chartKey: string;

  let canvas: HTMLCanvasElement;
  let InterHOAttemptChart: Chart;
  ...
  function updateChart(chartKey: string) {
    ...
    const RegionalINTERHOAttempt = processAPIData(InterFreqHORegionalData, chartKey)
    if (!RegionalINTERHOAttempt) return;

    const {band, ratio} = RegionalINTERHOAttempt
    const backgroundColors = band.map((bandName: string) => {
      return regionColors[bandName] || '#cccccc'
    });


    if (InterHOAttemptChart) {
      InterHOAttemptChart.data.labels = band;
      InterHOAttemptChart.data.datasets[0].data = ratio;
      InterHOAttemptChart.data.datasets[0].backgroundColor = backgroundColors;
      InterHOAttemptChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !InterHOAttemptChart) {
      Chart.register(ChartDataLabels);

      InterHOAttemptChart = new Chart(ctx, {
