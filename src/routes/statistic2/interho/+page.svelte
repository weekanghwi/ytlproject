<script lang="ts">
  ...
  import OverallInterfreqHOcount from './chart/OverallInterfreqHOcount.svelte';
  import OverallInterFreqHoAttempt from './chart/OverallInterFreqHOAttempt.svelte';
  import OverallInterFreqHoTrend from './chart/OverallInterFreqHOTrend.svelte';
  import OverallInterFreqHoTrendbyBand from './chart/OverallInterFreqHOTrendbyBand.svelte';
  import OverallInterFreqHoFailCluster from './chart/OverallInterFreqHOFailCluster.svelte';
  import OverallInterFreqHOFailSite from './chart/OverallInterFreqHOFailSite.svelte';
  import OverallInterFreqHoFailCell from './chart/OverallInterFreqHOFailCell.svelte';
  ...
  onMount(async () => {
    try {
      OverallData = data.InterFreqHOStatisticData.InterFreqHO;
    } catch (error) {
  ...
  <div class="flex flex-col gap-2 mb-4">
    <div class="flex items-center gap-2">
      <p class="text-xs text-slate-400 ms-1">
        {data.InterFreqHOStatisticData.InterFreqHO.interfreqho_trend_all.weeknums[9]} INTER FREQUENCY HANDOVER
      </p>
    </div>
  ...
  <div class="grid grid-cols-8 gap-4 mb-4">
    ...
    <div class="flex flex-col col-span-2 rounded-md bg-slate-900/40 p-4">
      <div class="flex items-center gap-1 mb-4">
        ...
        <h1 class="text-xs text-slate-500">INTER FREQUENCY HO ATTEMPT COUNT</h1>
      </div>
      <div class="flex items-center justify-around">
        <div class="relative flex items-center justify-center">
          <div class="z-50">
            <OverallInterfreqHOcount {OverallData} />
          </div>
          <div class="absolute flex flex-col items-center">
            <h1 class="text-xs text-slate-300">Inter Freq HO Count</h1>
            <h1 class="text-XS text-white font-bold">
              {(data.InterFreqHOStatisticData.InterFreqHO.TotalInterHONominate_byband.overall.TotalInterHONominate).toLocaleString()}
            </h1>
          </div>
        </div>
        ...
        <div class="flex flex-col justify-center gap-2">
          <div class="flex flex-col gap-1">
            {#each Object.keys(data.InterFreqHOStatisticData.InterFreqHO.TotalInterHONominate_byband.by_band.band) as index}
            <div class="flex items-center gap-1">
              <div class="h-3 w-3 rounded-sm 
              {data.InterFreqHOStatisticData.InterFreqHO.TotalInterHONominate_byband.by_band.band[index] === '800M' ? 'bg-sky-400' : 
              data.InterFreqHOStatisticData.InterFreqHO.TotalInterHONominate_byband.by_band.band[index] === '2.6GHz' ? 'bg-sky-600' : 'bg-sky-800'}"></div>
              <h1 class="text-xs text-white">{data.InterFreqHOStatisticData.InterFreqHO.TotalInterHONominate_byband.by_band.band[index]}</h1>
            </div>
            <h1 class="text-xs text-slate-500 ps-4">
              {(data.InterFreqHOStatisticData.InterFreqHO.TotalInterHONominate_byband.by_band.ratio[index]).toFixed(2)}%
            </h1>
            {/each}
          </div>
        </div>
    ...
    <!-- OVERALL INTER FREQ NOMINATE ITEM -->
    <div class="flex flex-col col-span-3 rounded-md bg-slate-900/40 p-4">
      <div class="flex items-center justify-between mb-4">
        <div class="flex items-center gap-1">
          <Icon icon="f7:number-square" class="text-lg text-slate-500" />
          <h1 class="text-xs text-slate-500">INTER FREQ HO Nominate Items Count & Ratio</h1>
        </div>
      ...
      <div class="flex items-center justify-center gap-4">
        <div class="flex flex-col justify-center w-2/5">
          <h1 class="text-3xl text-white font-bold mb-2">
            ...
            {(((SelectedNominateOverallData)/(data.InterFreqHOStatisticData.InterFreqHO.TotalInterHONominate_byband.overall.TotalInterHONominate)) * 100).toFixed(2)}%
            {/if}
          </h1>
          <h1 class="text-xs text-slate-400 font-bold mb-1">{interFreqHONominateKeyMap[SelectedinterFreqHONominateChartKey]}</h1>
          <h1 class="text-xs text-slate-500">
            {interFreqHONominateDescription[interFreqHONominateKeyMap[SelectedinterFreqHONominateChartKey]] || ''}
          </h1>
        </div>
        <div class="flex items-center justify-center gap-4">
          <div class="relative flex items-center justify-center">
            <div class="z-50">
              <OverallInterFreqHoAttempt {OverallData} chartKey={SelectedinterFreqHONominateChartKey} />
            </div>
            <div class="absolute flex flex-col items-center justify-center">
              <h1 class="text-xs text-slate-300">
                {interFreqHONominateKeyMap[SelectedinterFreqHONominateChartKey]}
              </h1>
              <h1 class="text-xs text-white font-bold">
    ...
    <div class="flex flex-col col-span-2 rounded-md bg-slate-900/40">
      <div class="relative flex items-center gap-1 p-4">
        ...
        <h1 class="text-xs text-slate-500">INTER FREQ HO WEEKLY TREND</h1>
        <div class="absolute top-10 left-5 flex items-center">
          <h1 class="text-2xl text-white font-bold">
            {(data.InterFreqHOStatisticData.InterFreqHO.interfreqho_trend_all.values[9]).toFixed(2)}%
          </h1>
          <p class="{`text-xs ps-2 ${(data.InterFreqHOStatisticData.InterFreqHO.interfreqho_trend_all.values[9] - data.InterFreqHOStatisticData.InterFreqHO.interfreqho_trend_all.values[8]) < 0 ? 'text-rose-400' : 'text-green-400' }`}">
            {(data.InterFreqHOStatisticData.InterFreqHO.interfreqho_trend_all.values[9] - data.InterFreqHOStatisticData.InterFreqHO.interfreqho_trend_all.values[8]).toFixed(2)}
          </p>
          {#if (data.InterFreqHOStatisticData.InterFreqHO.interfreqho_trend_all.values[9] - data.InterFreqHOStatisticData.InterFreqHO.interfreqho_trend_all.values[8]) > 0}
      ...
      <OverallInterFreqHoTrend {OverallData} />
    </div>

    <!-- INTER FREQ HO TREND BY BAND -->
    <div class="flex flex-col col-span-3 rounded-md bg-slate-900/40">
      ...
      <OverallInterFreqHoTrendbyBand {OverallData} />
    </div>

    <!-- INTER FREQ HO CLUSTER, SITE AND CELL COUNT -->
    <div class="flex flex-col col-span-3 rounded-md bg-slate-900/40 p-4">
      <div class="flex items-center gap-1 mb-4">
      ...
      <div class="flex items-center justify-around gap-3">
        <div class="relative flex items-center justify-center">
          <div class="z-50">
            <OverallInterFreqHoFailCluster {OverallData} />
          </div>
          <div class="absolute flex flex-col items-center justify-center">
            <h1 class="text-xs text-slate-400">Fail cluster</h1>
            <h1 class="text-2xl text-white font-bold">
              {(data.InterFreqHOStatisticData.InterFreqHO.fail_cluster_count_all).toLocaleString()}
            </h1>
          </div>
        </div>
        ...
        <div class="relative flex items-center justify-center">
          <div class="z-50">
            <OverallInterFreqHOFailSite {OverallData} />
          </div>
          <div class="absolute flex flex-col items-center justify-center">
            <h1 class="text-xs text-slate-400">Fail cluster</h1>
            <h1 class="text-2xl text-white font-bold">
              {(data.InterFreqHOStatisticData.InterFreqHO.fail_site_count_all).toLocaleString()}
            </h1>
          </div>
        </div>
        ...
        <div class="relative flex items-center justify-center">
          <div class="z-50">
            <OverallInterFreqHoFailCell {OverallData} />
          </div>
          <div class="absolute flex flex-col items-center justify-center">
            <h1 class="text-xs text-slate-400">Fail cluster</h1>
            <h1 class="text-2xl text-white font-bold">
              {(data.InterFreqHOStatisticData.InterFreqHO.fail_cell_count_all).toLocaleString()}
            </h1>
          </div>
        </div>
