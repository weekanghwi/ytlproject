<script lang="ts">
  ...
  let CellAvailOverallTrendChart: Chart;
  ...
  function updateChart() {
    const TrendData = OverallData.cellavail_trend_all;
    if (!TrendData) return;
    const { weeknums, values } = TrendData;

    if (CellAvailOverallTrendChart) {
      CellAvailOverallTrendChart.data.labels = weeknums;
      CellAvailOverallTrendChart.data.datasets[0].data = values;
      CellAvailOverallTrendChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !CellAvailOverallTrendChart) {
      Chart.register(ChartDataLabels);

      CellAvailOverallTrendChart = new Chart(ctx, {
