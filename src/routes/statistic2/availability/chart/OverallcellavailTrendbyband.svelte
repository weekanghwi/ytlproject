<script lang="ts">
  ...
  let CellAvailOverallTrendChartbyBand: Chart;
  ...
  function updateChart() {
    const TrendData = OverallData.cellavail_trend_23;
    if (!TrendData) return;
    ...
    const datasets = [
      OverallData.cellavail_trend_23,
      OverallData.cellavail_trend_26,
      OverallData.cellavail_trend_800,
    ].filter(Boolean);
    ...
    if (CellAvailOverallTrendChartbyBand) {
      CellAvailOverallTrendChartbyBand.options.scales.y = {
        display: true,
        beginAtZero: false,
        min: yAxisMin,
        // max: yAxisMax,
      ...
      CellAvailOverallTrendChartbyBand.data.labels = weeknums;
      CellAvailOverallTrendChartbyBand.data.datasets[0] = {
        label: "2.3G Trend",
        data: values,
      ...
      const secondLineData = OverallData.cellavail_trend_26; // 
      const secondDataset = {
        label: "2.6G Trend",
      ...
      const thirdLineData = OverallData.cellavail_trend_800;
      const thirdDataset = {
        label: "800M Trend",
      ...
      // 데이터셋 배열 업데이트
      CellAvailOverallTrendChartbyBand.data.datasets = [CellAvailOverallTrendChartbyBand.data.datasets[0], secondDataset, thirdDataset];

      // 차트 업데이트
      CellAvailOverallTrendChartbyBand.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !CellAvailOverallTrendChartbyBand) {
      Chart.register(ChartDataLabels);

      CellAvailOverallTrendChartbyBand = new Chart(ctx, {
