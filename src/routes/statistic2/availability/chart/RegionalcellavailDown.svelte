<script lang="ts">
  ...
  export let CellAvailRegionalData: any = null;
  export let chartKey: string;

  let canvas: HTMLCanvasElement;
  let CellAvailDownChart: Chart;
  ...
  function updateChart(chartKey: string) {
    const RegionalData = processAPIData(CellAvailRegionalData, chartKey)
    ...
    if (CellAvailDownChart) {
      CellAvailDownChart.data.labels = band;
      CellAvailDownChart.data.datasets[0].data = ratio;
      CellAvailDownChart.data.datasets[0].backgroundColor = backgroundColors;
      CellAvailDownChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !CellAvailDownChart) {
      Chart.register(ChartDataLabels);

      CellAvailDownChart = new Chart(ctx, {
