<script lang="ts">
  ...
  import { trendDataStore, fetchTrendData } from "./FetchCellAvailData";
	import { Tooltip } from "flowbite-svelte";


  import RegionalcellavailDown from "../chart/RegionalcellavailDown.svelte";
  import RegionalcellavailTrendbyBand from "../chart/RegionalcellavailTrendbyBand.svelte";
  import RegionalcellavailFailCluster from "../chart/RegionalcellavailFailCluster.svelte";
  import RegionalcellavailFailSiteCell from "../chart/RegionalcellavailFailSiteCell.svelte";
  ...
  let field: string = 'cell_availability';
  let CellAvailRegionalData: any = null;
  let band: string;

  $: CellAvailRegionalData = $trendDataStore
  $: $trendDataStore, CellAvailRegionalData = $trendDataStore
  $: if (region) {
    (region);
  }
  ...
  const CellAvailNominateCountChartKeyMap: { [key: string]: string} = {
    'cellavaildownbyband': 'Total Un Available',
  };
  const chartKeys = Object.keys(CellAvailNominateCountChartKeyMap);
  ...
  $: selectedNominateData = getSelectedData(CellAvailRegionalData, selectedChartKey)
  ...
  $: SelectedNominateOverallData = getSelectedNominateOverallData(CellAvailRegionalData, selectedChartKey)
...
<div class="grid grid-cols-8 gap-4 w-full mb-4">
  ...
  <div class="flex flex-col col-span-2 bg-slate-900/40 rounded-md p-4">
    <div class="flex items-center gap-1">
      ...
      <h1 class="text-xs text-slate-500">{region.toLocaleUpperCase()} CELL UN AVAILABLE TIME</h1>
    </div>

    {#if CellAvailRegionalData}
      <div class="flex flex-col items-center justify-center h-full">
        <div class="flex flex-col">
          <h1 class="text-2xl text-white font-bold">
            {((CellAvailRegionalData.cellavaildownbyband.overall.DownTime) - 
            (CellAvailRegionalData.cellavaildownbyband.overall.LockTime)).toLocaleString()}
          </h1>
          <h1 class="text-xs text-slate-500">
            {(((CellAvailRegionalData.cellavaildownbyband.overall.DownTime) - 
              (CellAvailRegionalData.cellavaildownbyband.overall.LockTime)) / 7).toLocaleString()} (Per Day)
          </h1>
        </div>
      </div>
      ...
      <div class="flex items-center gap-1">
        <div class="rounded-sm px-1 py-[1px] bg-green-500 text-xs">
          {
            ((((CellAvailRegionalData.cellavaildownbyband.overall.DownTime) - 
              (CellAvailRegionalData.cellavaildownbyband.overall.LockTime)) / 
              ((data.CellAvailStatisticData.CellAvail.cellavaildownbyband.overall.DownTime) - 
              (data.CellAvailStatisticData.CellAvail.cellavaildownbyband.overall.LockTime))) * 
              100).toFixed(1)
          }%
        </div>
        <h1 class="text-xs text-slate-500">Share of the national un available time</h1>
      </div>
    {/if}
  </div>

  <!-- REGIONAL CELL UN AVAILABLE BY BAND -->
  <div class="flex flex-col col-span-2 bg-slate-900/40 rounded-md p-4 gap-4">
    <div class="relative flex items-start gap-1">
      <Icon icon="fluent:calendar-week-numbers-24-regular" class="text-lg text-slate-500" />
      <div class="flex flex-col">
        <h1 class="text-xs text-slate-500">CELL UN AVAILABLE BY BAND</h1>
      </div>
      ...
      {#if showDropdown}
        <div class="z-50 absolute -top-5 right-5 flex flex-col bg-slate-700/90 shadow-md rounded-md mt-2">
          ...
          on:click={() => {selectedChartKey = key; showDropdown = false}}>
            {CellAvailNominateCountChartKeyMap[key]}
          </button>
          {/each}
        </div>
      {/if}
    ...
    <div class="flex items-center gap-4">
      <div class="relative flex items-center justify-center">
        <div class="z-10">
          <RegionalcellavailDown {CellAvailRegionalData} chartKey={selectedChartKey} />
        </div>
        <div class="absolute flex flex-col items-center justify-center gap-1">
          ...
          <h1 class="text-xs text-white">{CellAvailNominateCountChartKeyMap[selectedChartKey]}</h1>
        </div>
      </div>
  ...
  <!-- REGIONAL CELL AVAIL TREND BY BAND -->
  <div class="flex flex-col col-span-4 bg-slate-900/40 rounded-md p-4">
    <div class="flex items-center gap-1 mb-4">
      <Icon icon="fluent:calendar-week-numbers-24-regular" class="text-lg text-slate-500" />
      <h1 class="text-xs text-slate-500">{region.toLocaleUpperCase()} CELL AVAIL TREND BY BAND</h1>
    </div>
    <div class="">
      <RegionalcellavailTrendbyBand {CellAvailRegionalData} />
    </div>
  </div>

  <!-- REGIONAL CELL UN AVAIL CLUSTER -->
  <div class="flex flex-col col-span-2 bg-slate-900/40 rounded-md p-4">
    <div class="flex items-center gap-1 mb-4">
      <Icon icon="fluent:calendar-week-numbers-24-regular" class="text-lg text-slate-500" />
      <h1 class="text-xs text-slate-500">{region.toLocaleUpperCase()} CELL AVAIL FAIL CLUSTER</h1>
    </div>
    <div class="flex items-center justify-around">
      <div class="relative flex items-center justify-center">
        <RegionalcellavailFailCluster {CellAvailRegionalData} />
        <div class="absolute flex flex-col items-center justify-center">
          <h1 class="text-xs text-slate-500">Fail Cluster</h1>
          <h1 class="text-2xl text-white font-bold">
            {#if CellAvailRegionalData}{CellAvailRegionalData.fail_cluster_count_all}{/if}
          </h1>
        </div>
      </div>
      ...
      <div class="flex flex-col gap-2">
        <div class="flex flex-col">
          <div class="flex flex-col gap-4">
            {#if CellAvailRegionalData && CellAvailRegionalData[FailClusterCount]}
              {#each CellAvailRegionalData[FailClusterCount].band as bands, index}
                <div class="flex items-center">
                  <button class="flex items-center justify-between p-2 gap-2 text-xs text-white w-full rounded-md 
                    ...
                    <h1 class="text-xs text-black w-4 h-4 rounded-full bg-rose-300">
                      {CellAvailRegionalData[FailClusterCount].count[index]}
                    </h1>
                  </button>
                </div>
  ...
  <!-- REGIONAL  CELL UN AVAIL SITE & CELL -->
  <div class="flex flex-col col-span-3 bg-slate-900/40 rounded-md p-4">
    <div class="flex items-center gap-1 mb-4">
      <Icon icon="fluent:calendar-week-numbers-24-regular" class="text-lg text-slate-500" />
      <h1 class="text-xs text-slate-500">{region.toLocaleUpperCase()} CELL UN AVAIL SITE & CELL</h1>
    </div>
    <div class="flex items-center justify-around">
      <div class="relative flex items-center justify-center">
        <RegionalcellavailFailSiteCell {CellAvailRegionalData} />
        <div class="absolute flex flex-col items-center justify-center">
          <h1 class="text-xs text-slate-500">Site / Cell Fail</h1>
          <h1 class="text-lg text-white font-bold">
            {#if CellAvailRegionalData}
              {CellAvailRegionalData.fail_site_count_all}/{CellAvailRegionalData.fail_cell_count_all}
            {/if}
          </h1>
        </div>
      </div>
      <div class="grid grid-cols-3 gap-2">
        ...
        <div class="flex items-center justify-center w-full px-1 py-[1px] rounded-sm bg-rose-400">
          <h1 class="text-xs text-white">
            {#if CellAvailRegionalData}
              {#if CellAvailRegionalData.fail_site_count_byband.band.includes('2.3GHz')}
                {CellAvailRegionalData.fail_site_count_byband.count[CellAvailRegionalData.fail_site_count_byband.band.indexOf('2.3GHz')]}
              {:else}
              -
              {/if}
            {/if}
        ...
        <div class="flex items-center justify-center w-full px-1 py-[1px] rounded-sm bg-rose-400">
          <h1 class="text-xs text-white">
            {#if CellAvailRegionalData}
              {#if CellAvailRegionalData.fail_cell_count_byband.band.includes('2.3GHz')}
                {CellAvailRegionalData.fail_cell_count_byband.count[CellAvailRegionalData.fail_site_count_byband.band.indexOf('2.3GHz')]}
              {:else}
              -
              {/if}
            {/if}
        ...
        <div class="flex items-center justify-center w-full px-1 py-[1px] rounded-sm bg-rose-400">
          <h1 class="text-xs text-white">
            {#if CellAvailRegionalData}
              {#if CellAvailRegionalData.fail_site_count_byband.band.includes('2.6GHz')}
                {CellAvailRegionalData.fail_site_count_byband.count[CellAvailRegionalData.fail_site_count_byband.band.indexOf('2.6GHz')]}
              {:else}
              -
              {/if}
            {/if}
        ...
        <div class="flex items-center justify-center w-full px-1 py-[1px] rounded-sm bg-rose-400">
          <h1 class="text-xs text-white">
            {#if CellAvailRegionalData}
              {#if CellAvailRegionalData.fail_cell_count_byband.band.includes('2.6GHz')}
                {CellAvailRegionalData.fail_cell_count_byband.count[CellAvailRegionalData.fail_site_count_byband.band.indexOf('2.6GHz')]}
              {:else}
              -
              {/if}
            {/if}
        ...
        <div class="flex items-center justify-center w-full px-1 py-[1px] rounded-sm bg-rose-400">
          <h1 class="text-xs text-white">
            {#if CellAvailRegionalData}
              {#if CellAvailRegionalData.fail_site_count_byband.band.includes('800M')}
                {CellAvailRegionalData.fail_site_count_byband.count[CellAvailRegionalData.fail_site_count_byband.band.indexOf('800M')]}
              {:else}
              -
              {/if}
            {/if}
        ...
        <div class="flex items-center justify-center w-full px-1 py-[1px] rounded-sm bg-rose-400">
          <h1 class="text-xs text-white">
            {#if CellAvailRegionalData}
              {#if CellAvailRegionalData.fail_cell_count_byband.band.includes('800M')}
                {CellAvailRegionalData.fail_cell_count_byband.count[CellAvailRegionalData.fail_site_count_byband.band.indexOf('800M')]}
              {:else}
              -
              {/if}
            {/if}
  ...
  <!-- REGIONAL CELL AVAIL MOST FAIL CLUSTER AND CELL LIST -->
  <div class="flex flex-col col-span-3 bg-slate-900/40 rounded-md">
    ...
    <div class="flex items-center rounded-md pb-2">
      <table class="w-full">
        ...
        <tbody>
          {#if CellAvailRegionalData && CellAvailRegionalData[topImpactClusterListKey]}
            {#each Object.entries(CellAvailRegionalData[topImpactClusterListKey]) as [band, data]}
              {#each data.cluster as cluster, index}
                <tr class="text-xs text-slate-300 py-2">
                  <td class="py-1 px-4">{cluster}</td>
                  <td class="py-1 px-4">{(data.cellavailfailtotal[index]).toLocaleString()}</td>
                  <td class="py-1 px-4">{((data.cellavailfailtotal[index] / CellAvailRegionalData.cellavailfallbybandall.by_band.failcount[2]) * 100).toFixed(2)}%</td>
  ...
  <div class="flex flex-col col-span-3 bg-slate-900/40 rounded-md">
    <div class="flex items-center p-4 gap-1">
      ...
      <h1 class="text-xs text-slate-500">CELL AVAIL CLUSTER</h1>
    </div>
  ...
  <div class="flex flex-col col-span-5 bg-slate-900/40 rounded-md">
    <div class="flex items-center p-4 gap-1">
      ...
      <h1 class="text-xs text-slate-500">CELL AVAIL CELL</h1>
    </div>
