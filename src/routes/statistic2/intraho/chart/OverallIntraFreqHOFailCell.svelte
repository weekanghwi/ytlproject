<script lang="ts">
  ...
  let InterFreqHOCellChart: Chart;
  ...
  function updateChart() {
    ...
    if (InterFreqHOCellChart) {
      InterFreqHOCellChart.data.labels = band;
      InterFreqHOCellChart.data.datasets[0].data = count;
      InterFreqHOCellChart.data.datasets[0].backgroundColor = backgroundColors;
      InterFreqHOCellChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !InterFreqHOCellChart) {
      Chart.register(ChartDataLabels);

      InterFreqHOCellChart = new Chart(ctx, {
