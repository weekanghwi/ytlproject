<script lang="ts">
  ...
  let IntraFreqHOOverallTrendChartbyBand: Chart;
  ...
  function updateChart() {
    const TrendData = OverallData.intrafreqho_trend_23;
    if (!TrendData) return;
    ...
    const datasets = [
      OverallData.intrafreqho_trend_23,
      OverallData.intrafreqho_trend_26,
      OverallData.intrafreqho_trend_800,
    ].filter(Boolean);
    ...
    if (IntraFreqHOOverallTrendChartbyBand) {
      IntraFreqHOOverallTrendChartbyBand.options.scales.y = {
        display: true,
        beginAtZero: false,
        min: yAxisMin,
        // max: yAxisMax,
      ...
      IntraFreqHOOverallTrendChartbyBand.data.labels = weeknums;
      IntraFreqHOOverallTrendChartbyBand.data.datasets[0] = {
        label: "2.3G Trend",
        data: values,
      ...
      const secondLineData = OverallData.intrafreqho_trend_26; // 
      const secondDataset = {
        label: "2.6G Trend",
      ...
      const thirdLineData = OverallData.intrafreqho_trend_800;
      const thirdDataset = {
        label: "800M Trend",
      ...
      // 데이터셋 배열 업데이트
      IntraFreqHOOverallTrendChartbyBand.data.datasets = [IntraFreqHOOverallTrendChartbyBand.data.datasets[0], secondDataset, thirdDataset];

      // 차트 업데이트
      IntraFreqHOOverallTrendChartbyBand.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !IntraFreqHOOverallTrendChartbyBand) {
      Chart.register(ChartDataLabels);

      IntraFreqHOOverallTrendChartbyBand = new Chart(ctx, {
