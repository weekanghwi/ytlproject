<script lang="ts">
  ...
  let IntraFreqHOOverallTrendChart: Chart;
  ...
  function updateChart() {
    const TrendData = OverallData.intrafreqho_trend_all;
    if (!TrendData) return;
    const { weeknums, values } = TrendData;

    if (IntraFreqHOOverallTrendChart) {
      IntraFreqHOOverallTrendChart.data.labels = weeknums;
      IntraFreqHOOverallTrendChart.data.datasets[0].data = values;
      IntraFreqHOOverallTrendChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !IntraFreqHOOverallTrendChart) {
      Chart.register(ChartDataLabels);

      IntraFreqHOOverallTrendChart = new Chart(ctx, {
