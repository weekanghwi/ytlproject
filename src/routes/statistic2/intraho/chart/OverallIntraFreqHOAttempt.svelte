<script lang="ts">
  ...
  let InterFreqHOAttemptItemChartByBand: Chart;
  ...
  function updateChart(chartKey: string) {
    ...
    const InterFreqHOAttemptItembyBand = processAPIData(OverallData, chartKey)
    if (!InterFreqHOAttemptItembyBand) return;
    const {band, ratio} = InterFreqHOAttemptItembyBand
    ...
    if (InterFreqHOAttemptItemChartByBand) {
      InterFreqHOAttemptItemChartByBand.data.labels = band;
      InterFreqHOAttemptItemChartByBand.data.datasets[0].data = ratio;
      InterFreqHOAttemptItemChartByBand.data.datasets[0].backgroundColor = backgroundColors;
      InterFreqHOAttemptItemChartByBand.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !InterFreqHOAttemptItemChartByBand) {
      Chart.register(ChartDataLabels);

      InterFreqHOAttemptItemChartByBand = new Chart(ctx, {
