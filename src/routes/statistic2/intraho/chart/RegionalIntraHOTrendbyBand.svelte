<script lang="ts">
  ...
  export let IntraFreqHORegionalData: any = null; // 부모 컴포넌트에서 전달받은 데이터

  let canvas: HTMLCanvasElement;
  let IntraHORegionalTrendChartbyBand: Chart;
  ...
  function updateChart() {
    const RegionalTrendData = IntraFreqHORegionalData.regional_intrafreqho_trend_23;
    ...
    const datasets = [
      IntraFreqHORegionalData.regional_intrafreqho_trend_23,
      IntraFreqHORegionalData.regional_intrafreqho_trend_26,
      IntraFreqHORegionalData.regional_intrafreqho_trend_800,
    ].filter(Boolean);
    ...
    if (IntraHORegionalTrendChartbyBand) {
      IntraHORegionalTrendChartbyBand.options.scales.y = {
        display: true,
        beginAtZero: false,
        min: yAxisMin,
        // max: yAxisMax,
      ...
      IntraHORegionalTrendChartbyBand.data.labels = weeknums;
      IntraHORegionalTrendChartbyBand.data.datasets[0] = {
        label: "2.3G Trend",
        data: values,
      ...
      const secondLineData = IntraFreqHORegionalData.regional_intrafreqho_trend_26; // 
      const secondDataset = {
        label: "2.6G Trend",
      ...
      const thirdLineData = IntraFreqHORegionalData.regional_intrafreqho_trend_800;
      const thirdDataset = {
        label: "800M Trend",
      ...
      // 데이터셋 배열 업데이트
      IntraHORegionalTrendChartbyBand.data.datasets = [IntraHORegionalTrendChartbyBand.data.datasets[0], secondDataset, thirdDataset];

      // 차트 업데이트
      IntraHORegionalTrendChartbyBand.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !IntraHORegionalTrendChartbyBand) {
      Chart.register(ChartDataLabels);

      IntraHORegionalTrendChartbyBand = new Chart(ctx, {
