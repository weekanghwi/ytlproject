<script lang="ts">
  ...
  function updateChart() {
    ...
    if (x2InHOClusterChart) {
      x2InHOClusterChart.data.labels = band;
      x2InHOClusterChart.data.datasets[0].data = count;
      x2InHOClusterChart.data.datasets[0].backgroundColor = backgroundColors;
      x2InHOClusterChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !x2InHOClusterChart) {
      Chart.register(ChartDataLabels);

      x2InHOClusterChart = new Chart(ctx, {
