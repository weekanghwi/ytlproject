<script lang="ts">
  ...
  function updateChart() {
    ...
    if (x2InHOSiteChart) {
      x2InHOSiteChart.data.labels = band;
      x2InHOSiteChart.data.datasets[0].data = count;
      x2InHOSiteChart.data.datasets[0].backgroundColor = backgroundColors;
      x2InHOSiteChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !x2InHOSiteChart) {
      Chart.register(ChartDataLabels);

      x2InHOSiteChart = new Chart(ctx, {
