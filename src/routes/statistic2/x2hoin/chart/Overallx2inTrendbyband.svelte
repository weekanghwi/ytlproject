<script lang="ts">
  ...
  let x2InHOOverallTrendChartbyBand: Chart;
  ...
  function updateChart() {
    const TrendData = OverallData.x2inho_trend_23;
    if (!TrendData) return;
    ...
    const datasets = [
      OverallData.x2inho_trend_23,
      OverallData.x2inho_trend_26,
      OverallData.x2inho_trend_800,
    ].filter(Boolean);
    ...
    if (x2InHOOverallTrendChartbyBand) {
      x2InHOOverallTrendChartbyBand.options.scales.y = {
        display: true,
        beginAtZero: false,
        min: yAxisMin,
        // max: yAxisMax,
      ...
      x2InHOOverallTrendChartbyBand.data.labels = weeknums;
      x2InHOOverallTrendChartbyBand.data.datasets[0] = {
        label: "2.3G Trend",
        data: values,
      ...
      const secondLineData = OverallData.x2inho_trend_26; // 
      const secondDataset = {
        label: "2.6G Trend",
      ...
      const thirdLineData = OverallData.x2inho_trend_800;
      const thirdDataset = {
        label: "800M Trend",
      ...
      // 데이터셋 배열 업데이트
      x2InHOOverallTrendChartbyBand.data.datasets = [x2InHOOverallTrendChartbyBand.data.datasets[0], secondDataset, thirdDataset];

      // 차트 업데이트
      x2InHOOverallTrendChartbyBand.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !x2InHOOverallTrendChartbyBand) {
      Chart.register(ChartDataLabels);

      x2InHOOverallTrendChartbyBand = new Chart(ctx, {
