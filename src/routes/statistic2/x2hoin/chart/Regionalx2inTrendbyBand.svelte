<script lang="ts">
  ...
  export let x2InHORegionalData: any = null; // 부모 컴포넌트에서 전달받은 데이터

  let canvas: HTMLCanvasElement;
  let x2InHORegionalTrendChartbyBand: Chart;
  ...
  function updateChart() {
    const RegionalTrendData = x2InHORegionalData.regional_x2inho_trend_23;
    ...
    const datasets = [
      x2InHORegionalData.regional_x2inho_trend_23,
      x2InHORegionalData.regional_x2inho_trend_26,
      x2InHORegionalData.regional_x2inho_trend_800,
    ].filter(Boolean);
    ...
    if (x2InHORegionalTrendChartbyBand) {
      x2InHORegionalTrendChartbyBand.options.scales.y = {
        display: true,
        beginAtZero: false,
        min: yAxisMin,
        // max: yAxisMax,
      ...
      x2InHORegionalTrendChartbyBand.data.labels = weeknums;
      x2InHORegionalTrendChartbyBand.data.datasets[0] = {
        label: "2.3G Trend",
        data: values,
      ...
      const secondLineData = x2InHORegionalData.regional_x2inho_trend_26; // 
      const secondDataset = {
        label: "2.6G Trend",
      ...
      const thirdLineData = x2InHORegionalData.regional_x2inho_trend_800;
      const thirdDataset = {
        label: "800M Trend",
      ...
      // 데이터셋 배열 업데이트
      x2InHORegionalTrendChartbyBand.data.datasets = [x2InHORegionalTrendChartbyBand.data.datasets[0], secondDataset, thirdDataset];

      // 차트 업데이트
      x2InHORegionalTrendChartbyBand.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !x2InHORegionalTrendChartbyBand) {
      Chart.register(ChartDataLabels);

      x2InHORegionalTrendChartbyBand = new Chart(ctx, {
