<script lang="ts">
  ...
  let x2InHOOverallTrendChart: Chart;
  ...
  function updateChart() {
    const TrendData = OverallData.x2inho_trend_all;
    if (!TrendData) return;
    const { weeknums, values } = TrendData;

    if (x2InHOOverallTrendChart) {
      x2InHOOverallTrendChart.data.labels = weeknums;
      x2InHOOverallTrendChart.data.datasets[0].data = values;
      x2InHOOverallTrendChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !x2InHOOverallTrendChart) {
      Chart.register(ChartDataLabels);

      x2InHOOverallTrendChart = new Chart(ctx, {
