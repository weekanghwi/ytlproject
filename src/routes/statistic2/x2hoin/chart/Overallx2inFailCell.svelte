<script lang="ts">
  ...
  function updateChart() {
    ...
    if (x2InHOCellChart) {
      x2InHOCellChart.data.labels = band;
      x2InHOCellChart.data.datasets[0].data = count;
      x2InHOCellChart.data.datasets[0].backgroundColor = backgroundColors;
      x2InHOCellChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !x2InHOCellChart) {
      Chart.register(ChartDataLabels);

      x2InHOCellChart = new Chart(ctx, {
