<script lang="ts">
  ...
  import Overallx2inCount from './chart/Overallx2inCount.svelte';
  // // // import OverallInterFreqHoAttempt from './chart/OverallInterFreqHOAttempt.svelte';
  import Overallx2inTrend from './chart/Overallx2inTrend.svelte';
  import Overallx2inTrendbyband from './chart/Overallx2inTrendbyband.svelte';
  import Overallx2inFailCluster from './chart/Overallx2inFailCluster.svelte';
  import Overallx2inFailSite from './chart/Overallx2inFailSite.svelte';
  import Overallx2inFailCell from './chart/Overallx2inFailCell.svelte';
  ...
  onMount(async () => {
    try {
      OverallData = data.x2InHOStatisticData.x2InHO;
    } catch (error) {
  ...
  <div class="flex flex-col gap-2 mb-4">
    <div class="flex items-center gap-2">
      <p class="text-xs text-slate-400 ms-1">
        {data.x2InHOStatisticData.x2InHO.x2inho_trend_all.weeknums[9]} X2 IN HANDOVER
      </p>
    </div>
  ...
  <div class="grid grid-cols-8 gap-4 mb-4">
    ...
    <div class="flex flex-col col-span-2 rounded-md bg-slate-900/40 p-4">
      <div class="flex items-center gap-1 mb-4">
        ...
        <h1 class="text-xs text-slate-500">X2 IN HO ATTEMPT</h1>
      </div>
      <div class="flex items-center justify-around">
        <div class="relative flex items-center justify-center">
          <div class="z-50">
            <Overallx2inCount {OverallData} />
          </div>
          <div class="absolute flex flex-col items-center">
            <h1 class="text-xs text-slate-300">X2 IN HO Attempt</h1>
            <h1 class="text-XS text-white font-bold">
              {(data.x2InHOStatisticData.x2InHO.Totalx2InHONominate_byband.overall.Totalx2InHONominate).toLocaleString()}
            </h1>
          </div>
        </div>
        ...
        <div class="flex flex-col justify-center gap-2">
          <div class="flex flex-col gap-1">
            {#each Object.keys(data.x2InHOStatisticData.x2InHO.Totalx2InHONominate_byband.by_band.band) as index}
            <div class="flex items-center gap-1">
              <div class="h-3 w-3 rounded-sm 
              {data.x2InHOStatisticData.x2InHO.Totalx2InHONominate_byband.by_band.band[index] === '800M' ? 'bg-sky-400' : 
              data.x2InHOStatisticData.x2InHO.Totalx2InHONominate_byband.by_band.band[index] === '2.6GHz' ? 'bg-sky-600' : 'bg-sky-800'}"></div>
              <h1 class="text-xs text-white">{data.x2InHOStatisticData.x2InHO.Totalx2InHONominate_byband.by_band.band[index]}</h1>
            </div>
            <h1 class="text-xs text-slate-500 ps-4">
              {(data.x2InHOStatisticData.x2InHO.Totalx2InHONominate_byband.by_band.ratio[index]).toFixed(2)}%
            </h1>
            {/each}
          </div>
        </div>
    ...
    <!-- OVERALL X2 IN HO NOMINATE ITEM -->
    <div class="flex flex-col col-span-3 rounded-md bg-slate-900/40 p-4">
      <div class="flex items-center justify-between mb-4">
        <div class="flex items-center gap-1">
          <Icon icon="f7:number-square" class="text-lg text-slate-500" />
          <h1 class="text-xs text-slate-500">X2 IN HO Nominate Items Count & Ratio</h1>
        </div>
    ...
    <div class="flex flex-col col-span-2 rounded-md bg-slate-900/40">
      <div class="relative flex items-center gap-1 p-4">
        ...
        <h1 class="text-xs text-slate-500">X2 IN HO WEEKLY TREND</h1>
        <div class="absolute top-10 left-5 flex items-center">
          <h1 class="text-2xl text-white font-bold">
            {(data.x2InHOStatisticData.x2InHO.x2inho_trend_all.values[9]).toFixed(2)}%
          </h1>
          <p class="{`text-xs ps-2 ${(data.x2InHOStatisticData.x2InHO.x2inho_trend_all.values[9] - data.x2InHOStatisticData.x2InHO.x2inho_trend_all.values[8]) < 0 ? 'text-rose-400' : 'text-green-400' }`}">
            {(data.x2InHOStatisticData.x2InHO.x2inho_trend_all.values[9] - data.x2InHOStatisticData.x2InHO.x2inho_trend_all.values[8]).toFixed(2)}
          </p>
          {#if (data.x2InHOStatisticData.x2InHO.x2inho_trend_all.values[9] - data.x2InHOStatisticData.x2InHO.x2inho_trend_all.values[8]) > 0}
      ...
      <Overallx2inTrend {OverallData} />
    </div>

    <!-- X2 IN HO TREND BY BAND -->
    <div class="flex flex-col col-span-3 rounded-md bg-slate-900/40">
      ...
      <Overallx2inTrendbyband {OverallData} />
    </div>

    <!-- X2 IN HO CLUSTER, SITE AND CELL COUNT -->
    <div class="flex flex-col col-span-3 rounded-md bg-slate-900/40 p-4">
      <div class="flex items-center gap-1 mb-4">
      ...
      <div class="flex items-center justify-around gap-3">
        <div class="relative flex items-center justify-center">
          <div class="z-50">
            <Overallx2inFailCluster {OverallData} />
          </div>
          <div class="absolute flex flex-col items-center justify-center">
            <h1 class="text-xs text-slate-400">Fail cluster</h1>
            <h1 class="text-2xl text-white font-bold">
              {(data.x2InHOStatisticData.x2InHO.fail_cluster_count_all).toLocaleString()}
            </h1>
          </div>
        </div>
        ...
        <div class="relative flex items-center justify-center">
          <div class="z-50">
            <Overallx2inFailSite {OverallData} />
          </div>
          <div class="absolute flex flex-col items-center justify-center">
            <h1 class="text-xs text-slate-400">Fail cluster</h1>
            <h1 class="text-2xl text-white font-bold">
              {(data.x2InHOStatisticData.x2InHO.fail_site_count_all).toLocaleString()}
            </h1>
          </div>
        </div>
        ...
        <div class="relative flex items-center justify-center">
          <div class="z-50">
            <Overallx2inFailCell {OverallData} />
          </div>
          <div class="absolute flex flex-col items-center justify-center">
            <h1 class="text-xs text-slate-400">Fail cluster</h1>
            <h1 class="text-2xl text-white font-bold">
              {(data.x2InHOStatisticData.x2InHO.fail_cell_count_all).toLocaleString()}
            </h1>
          </div>
        </div>
