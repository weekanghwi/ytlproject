<script lang="ts">
  ...
  export let eRABSetupData: any = null; // 부모 컴포넌트에서 전달받은 데이터

  let canvas: HTMLCanvasElement;
  let eRABConnectionRegionalTrendChartbyBand: Chart;
  ...
  function updateChart() {
    const RegionalTrendData = eRABSetupData.regional_erabsetup_trend_23;
    ...
    const datasets = [
      eRABSetupData.regional_erabsetup_trend_23,
      eRABSetupData.regional_erabsetup_trend_26,
      eRABSetupData.regional_erabsetup_trend_800,
    ].filter(Boolean);
    ...
    if (eRABConnectionRegionalTrendChartbyBand) {
      eRABConnectionRegionalTrendChartbyBand.options.scales.y = {
        display: true,
        beginAtZero: false,
        min: yAxisMin,
        // max: yAxisMax,
      ...
      eRABConnectionRegionalTrendChartbyBand.data.labels = weeknums;
      eRABConnectionRegionalTrendChartbyBand.data.datasets[0] = {
        label: "2.3G Trend",
        data: values,
      ...
      const secondLineData = eRABSetupData.regional_erabsetup_trend_26; // 
      const secondDataset = {
        label: "2.6G Trend",
      ...
      const thirdLineData = eRABSetupData.regional_erabsetup_trend_800;
      const thirdDataset = {
        label: "800M Trend",
      ...
      // 데이터셋 배열 업데이트
      eRABConnectionRegionalTrendChartbyBand.data.datasets = [eRABConnectionRegionalTrendChartbyBand.data.datasets[0], secondDataset, thirdDataset];

      // 차트 업데이트
      eRABConnectionRegionalTrendChartbyBand.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !eRABConnectionRegionalTrendChartbyBand) {
      Chart.register(ChartDataLabels);

      eRABConnectionRegionalTrendChartbyBand = new Chart(ctx, {
