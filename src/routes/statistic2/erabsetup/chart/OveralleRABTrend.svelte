<script lang="ts">
  ...
  let eRABConnectionOverallTrendChart: Chart;
  ...
  function updateChart() {
    const TrendData = OverallData.erabsetup_trend_all;
    if (!TrendData) return;
    const { weeknums, values } = TrendData;

    if (eRABConnectionOverallTrendChart) {
      eRABConnectionOverallTrendChart.data.labels = weeknums;
      eRABConnectionOverallTrendChart.data.datasets[0].data = values;
      eRABConnectionOverallTrendChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !eRABConnectionOverallTrendChart) {
      Chart.register(ChartDataLabels);

      eRABConnectionOverallTrendChart = new Chart(ctx, {
