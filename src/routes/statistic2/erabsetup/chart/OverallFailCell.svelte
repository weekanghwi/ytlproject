<script lang="ts">
  ...
  let eRABOverallFailCellChart: Chart;
  ...
  function updateChart() {
    ...
    if (eRABOverallFailCellChart) {
      eRABOverallFailCellChart.data.labels = band;
      eRABOverallFailCellChart.data.datasets[0].data = count;
      eRABOverallFailCellChart.data.datasets[0].backgroundColor = backgroundColors;
      eRABOverallFailCellChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !eRABOverallFailCellChart) {
      Chart.register(ChartDataLabels);

      eRABOverallFailCellChart = new Chart(ctx, {
