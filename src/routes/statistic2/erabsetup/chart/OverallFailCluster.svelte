<script lang="ts">
  ...
  let eRABOverallFailClusterChart: Chart;
  ...
  function updateChart() {
    ...
    if (eRABOverallFailClusterChart) {
      eRABOverallFailClusterChart.data.labels = band;
      eRABOverallFailClusterChart.data.datasets[0].data = count;
      eRABOverallFailClusterChart.data.datasets[0].backgroundColor = backgroundColors;
      eRABOverallFailClusterChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !eRABOverallFailClusterChart) {
      Chart.register(ChartDataLabels);

      eRABOverallFailClusterChart = new Chart(ctx, {
