<script lang="ts">
  ...
  function updateChart(chartKey: string) {
    ...
    const VoLTEAttemptCountDatabyBand = processAPIData(CallDropGBRData, chartKey)
    if (!VoLTEAttemptCountDatabyBand) return;

    const {band, ratio} = VoLTEAttemptCountDatabyBand
