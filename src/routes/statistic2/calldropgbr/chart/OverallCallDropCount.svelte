<script lang="ts">
  ...
  let CallDropOverallChartByBand: Chart;
  ...
  function updateChart() {
    const CallDropCountbyBand = OverallData.calldropcoutbyband.by_band;
    ...
    if (CallDropOverallChartByBand) {
      CallDropOverallChartByBand.data.labels = band;
      CallDropOverallChartByBand.data.datasets[0].data = ratio;
      CallDropOverallChartByBand.data.datasets[0].backgroundColor = backgroundColors;
      CallDropOverallChartByBand.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !CallDropOverallChartByBand) {
      Chart.register(ChartDataLabels);

      CallDropOverallChartByBand = new Chart(ctx, {
