<script lang="ts">
  ...
  export let CallDropGBRData: any = null; // 부모 컴포넌트에서 전달받은 데이터
  ...
  function updateChart() {
    const RegionalTrendData = CallDropGBRData.regional_calldrop_trend_23;
    ...
    const datasets = [
      CallDropGBRData.regional_calldrop_trend_23,
      CallDropGBRData.regional_calldrop_trend_26,
      CallDropGBRData.regional_calldrop_trend_800,
    ].filter(Boolean);
    ...
    if (CallDropRegionalTrendChartbyBand) {
      ...
      const secondLineData = CallDropGBRData.regional_calldrop_trend_26; // 
      const secondDataset = {
        label: "2.6G Trend",
      ...
      const thirdLineData = CallDropGBRData.regional_calldrop_trend_800;
      const thirdDataset = {
        label: "800M Trend",
