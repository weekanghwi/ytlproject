<script lang="ts">
  ...
  function updateChart() {
    const CallDropDatabyBand = OverallData.fail_site_count_byband;
    ...
    if (CallDropSiteChart) {
      CallDropSiteChart.data.labels = band;
      CallDropSiteChart.data.datasets[0].data = count;
      CallDropSiteChart.data.datasets[0].backgroundColor = backgroundColors;
      CallDropSiteChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !CallDropSiteChart) {
      Chart.register(ChartDataLabels);

      CallDropSiteChart = new Chart(ctx, {
