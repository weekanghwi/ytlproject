<script lang="ts">
  ...
  let CallDropOverallTrendChart: Chart;
  ...
  function updateChart() {
    const TrendData = OverallData.calldrop_trend_all;
    if (!TrendData) return;
    const { weeknums, values } = TrendData;

    if (CallDropOverallTrendChart) {
      CallDropOverallTrendChart.data.labels = weeknums;
      CallDropOverallTrendChart.data.datasets[0].data = values;
      CallDropOverallTrendChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !CallDropOverallTrendChart) {
      Chart.register(ChartDataLabels);

      CallDropOverallTrendChart = new Chart(ctx, {
