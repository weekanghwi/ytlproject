<script lang="ts">
  ...
  import Overalls1inCount from './chart/Overalls1inCount.svelte';
  // // import OverallInterFreqHoAttempt from './chart/OverallInterFreqHOAttempt.svelte';
  import Overalls1inTrend from './chart/Overalls1inTrend.svelte';
  import Overalls1inTrendbyband from './chart/Overalls1inTrendbyband.svelte';
  import Overalls1inFailCluster from './chart/Overalls1inFailCluster.svelte';
  import Overalls1inFailSite from './chart/Overalls1inFailSite.svelte';
  import Overalls1inFailCell from './chart/Overalls1inFailCell.svelte';
  ...
  onMount(async () => {
    try {
      OverallData = data.s1InHOStatisticData.s1InHO;
    } catch (error) {
  ...
  <div class="flex flex-col gap-2 mb-4">
    <div class="flex items-center gap-2">
      <p class="text-xs text-slate-400 ms-1">
        {data.s1InHOStatisticData.s1InHO.s1inho_trend_all.weeknums[9]} S1 IN HANDOVER
      </p>
    </div>
  ...
  <div class="grid grid-cols-8 gap-4 mb-4">
    ...
    <div class="flex flex-col col-span-2 rounded-md bg-slate-900/40 p-4">
      <div class="flex items-center gap-1 mb-4">
        ...
        <h1 class="text-xs text-slate-500">S1 IN HO ATTEMPT</h1>
      </div>
      <div class="flex items-center justify-around">
        <div class="relative flex items-center justify-center">
          <div class="z-50">
            <Overalls1inCount {OverallData} />
          </div>
          <div class="absolute flex flex-col items-center">
            <h1 class="text-xs text-slate-300">S1 IN HO Attempt</h1>
            <h1 class="text-XS text-white font-bold">
              {(data.s1InHOStatisticData.s1InHO.Totals1InHONominate_byband.overall.Totals1InHONominate).toLocaleString()}
            </h1>
          </div>
        </div>
        ...
        <div class="flex flex-col justify-center gap-2">
          <div class="flex flex-col gap-1">
            {#each Object.keys(data.s1InHOStatisticData.s1InHO.Totals1InHONominate_byband.by_band.band) as index}
            <div class="flex items-center gap-1">
              <div class="h-3 w-3 rounded-sm 
              {data.s1InHOStatisticData.s1InHO.Totals1InHONominate_byband.by_band.band[index] === '800M' ? 'bg-sky-400' : 
              data.s1InHOStatisticData.s1InHO.Totals1InHONominate_byband.by_band.band[index] === '2.6GHz' ? 'bg-sky-600' : 'bg-sky-800'}"></div>
              <h1 class="text-xs text-white">{data.s1InHOStatisticData.s1InHO.Totals1InHONominate_byband.by_band.band[index]}</h1>
            </div>
            <h1 class="text-xs text-slate-500 ps-4">
              {(data.s1InHOStatisticData.s1InHO.Totals1InHONominate_byband.by_band.ratio[index]).toFixed(2)}%
            </h1>
            {/each}
          </div>
        </div>
    ...
    <!-- OVERALL S1 IN HO NOMINATE ITEM -->
    <div class="flex flex-col col-span-3 rounded-md bg-slate-900/40 p-4">
      <div class="flex items-center justify-between mb-4">
        <div class="flex items-center gap-1">
          <Icon icon="f7:number-square" class="text-lg text-slate-500" />
          <h1 class="text-xs text-slate-500">S1 IN HO Nominate Items Count & Ratio</h1>
        </div>
    ...
    <div class="flex flex-col col-span-2 rounded-md bg-slate-900/40">
      <div class="relative flex items-center gap-1 p-4">
        ...
        <h1 class="text-xs text-slate-500">S1 IN HO WEEKLY TREND</h1>
        <div class="absolute top-10 left-5 flex items-center">
          <h1 class="text-2xl text-white font-bold">
            {(data.s1InHOStatisticData.s1InHO.s1inho_trend_all.values[9]).toFixed(2)}%
          </h1>
          <p class="{`text-xs ps-2 ${(data.s1InHOStatisticData.s1InHO.s1inho_trend_all.values[9] - data.s1InHOStatisticData.s1InHO.s1inho_trend_all.values[8]) < 0 ? 'text-rose-400' : 'text-green-400' }`}">
            {(data.s1InHOStatisticData.s1InHO.s1inho_trend_all.values[9] - data.s1InHOStatisticData.s1InHO.s1inho_trend_all.values[8]).toFixed(2)}
          </p>
          {#if (data.s1InHOStatisticData.s1InHO.s1inho_trend_all.values[9] - data.s1InHOStatisticData.s1InHO.s1inho_trend_all.values[8]) > 0}
      ...
      <Overalls1inTrend {OverallData} />
    </div>

    <!-- S1 IN HO TREND BY BAND -->
    <div class="flex flex-col col-span-3 rounded-md bg-slate-900/40">
      ...
      <Overalls1inTrendbyband {OverallData} />
    </div>

    <!-- S1 IN HO CLUSTER, SITE AND CELL COUNT -->
    <div class="flex flex-col col-span-3 rounded-md bg-slate-900/40 p-4">
      <div class="flex items-center gap-1 mb-4">
      ...
      <div class="flex items-center justify-around gap-3">
        <div class="relative flex items-center justify-center">
          <div class="z-50">
            <Overalls1inFailCluster {OverallData} />
          </div>
          <div class="absolute flex flex-col items-center justify-center">
            <h1 class="text-xs text-slate-400">Fail cluster</h1>
            <h1 class="text-2xl text-white font-bold">
              {(data.s1InHOStatisticData.s1InHO.fail_cluster_count_all).toLocaleString()}
            </h1>
          </div>
        </div>
        ...
        <div class="relative flex items-center justify-center">
          <div class="z-50">
            <Overalls1inFailSite {OverallData} />
          </div>
          <div class="absolute flex flex-col items-center justify-center">
            <h1 class="text-xs text-slate-400">Fail cluster</h1>
            <h1 class="text-2xl text-white font-bold">
              {(data.s1InHOStatisticData.s1InHO.fail_site_count_all).toLocaleString()}
            </h1>
          </div>
        </div>
        ...
        <div class="relative flex items-center justify-center">
          <div class="z-50">
            <Overalls1inFailCell {OverallData} />
          </div>
          <div class="absolute flex flex-col items-center justify-center">
            <h1 class="text-xs text-slate-400">Fail cluster</h1>
            <h1 class="text-2xl text-white font-bold">
              {(data.s1InHOStatisticData.s1InHO.fail_cell_count_all).toLocaleString()}
            </h1>
          </div>
        </div>
