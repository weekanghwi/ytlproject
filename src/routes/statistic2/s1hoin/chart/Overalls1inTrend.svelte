<script lang="ts">
  ...
  let s1InHOOverallTrendChart: Chart;
  ...
  function updateChart() {
    const TrendData = OverallData.s1inho_trend_all;
    if (!TrendData) return;
    const { weeknums, values } = TrendData;

    if (s1InHOOverallTrendChart) {
      s1InHOOverallTrendChart.data.labels = weeknums;
      s1InHOOverallTrendChart.data.datasets[0].data = values;
      s1InHOOverallTrendChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !s1InHOOverallTrendChart) {
      Chart.register(ChartDataLabels);

      s1InHOOverallTrendChart = new Chart(ctx, {
