<script lang="ts">
  ...
  let s1InHOOverallTrendChartbyBand: Chart;
  ...
  function updateChart() {
    const TrendData = OverallData.s1inho_trend_23;
    if (!TrendData) return;
    ...
    const datasets = [
      OverallData.s1inho_trend_23,
      OverallData.s1inho_trend_26,
      OverallData.s1inho_trend_800,
    ].filter(Boolean);
    ...
    if (s1InHOOverallTrendChartbyBand) {
      s1InHOOverallTrendChartbyBand.options.scales.y = {
        display: true,
        beginAtZero: false,
        min: yAxisMin,
        // max: yAxisMax,
      ...
      s1InHOOverallTrendChartbyBand.data.labels = weeknums;
      s1InHOOverallTrendChartbyBand.data.datasets[0] = {
        label: "2.3G Trend",
        data: values,
      ...
      const secondLineData = OverallData.s1inho_trend_26; // 
      const secondDataset = {
        label: "2.6G Trend",
      ...
      const thirdLineData = OverallData.s1inho_trend_800;
      const thirdDataset = {
        label: "800M Trend",
      ...
      // 데이터셋 배열 업데이트
      s1InHOOverallTrendChartbyBand.data.datasets = [s1InHOOverallTrendChartbyBand.data.datasets[0], secondDataset, thirdDataset];

      // 차트 업데이트
      s1InHOOverallTrendChartbyBand.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !s1InHOOverallTrendChartbyBand) {
      Chart.register(ChartDataLabels);

      s1InHOOverallTrendChartbyBand = new Chart(ctx, {
