<script lang="ts">
  ...
  export let s1InHORegionalData: any = null;
  export let chartKey: string;

  let canvas: HTMLCanvasElement;
  let s1InHOAttemptChart: Chart;
  ...
  function updateChart(chartKey: string) {
    const RegionalData = processAPIData(s1InHORegionalData, chartKey)
    ...
    if (s1InHOAttemptChart) {
      s1InHOAttemptChart.data.labels = band;
      s1InHOAttemptChart.data.datasets[0].data = ratio;
      s1InHOAttemptChart.data.datasets[0].backgroundColor = backgroundColors;
      s1InHOAttemptChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !s1InHOAttemptChart) {
      Chart.register(ChartDataLabels);

      s1InHOAttemptChart = new Chart(ctx, {
