<script lang="ts">
  ...
  export let s1InHORegionalData: any = null; // 부모 컴포넌트에서 전달받은 데이터

  let canvas: HTMLCanvasElement;
  let s1InHORegionalTrendChartbyBand: Chart;
  ...
  function updateChart() {
    const RegionalTrendData = s1InHORegionalData.regional_s1inho_trend_23;
    ...
    const datasets = [
      s1InHORegionalData.regional_s1inho_trend_23,
      s1InHORegionalData.regional_s1inho_trend_26,
      s1InHORegionalData.regional_s1inho_trend_800,
    ].filter(Boolean);
    ...
    if (s1InHORegionalTrendChartbyBand) {
      s1InHORegionalTrendChartbyBand.options.scales.y = {
        display: true,
        beginAtZero: false,
        min: yAxisMin,
        // max: yAxisMax,
      ...
      s1InHORegionalTrendChartbyBand.data.labels = weeknums;
      s1InHORegionalTrendChartbyBand.data.datasets[0] = {
        label: "2.3G Trend",
        data: values,
      ...
      const secondLineData = s1InHORegionalData.regional_s1inho_trend_26; // 
      const secondDataset = {
        label: "2.6G Trend",
      ...
      const thirdLineData = s1InHORegionalData.regional_s1inho_trend_800;
      const thirdDataset = {
        label: "800M Trend",
      ...
      // 데이터셋 배열 업데이트
      s1InHORegionalTrendChartbyBand.data.datasets = [s1InHORegionalTrendChartbyBand.data.datasets[0], secondDataset, thirdDataset];

      // 차트 업데이트
      s1InHORegionalTrendChartbyBand.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !s1InHORegionalTrendChartbyBand) {
      Chart.register(ChartDataLabels);

      s1InHORegionalTrendChartbyBand = new Chart(ctx, {
