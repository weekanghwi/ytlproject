<script lang="ts">
  ...
  function updateChart() {
    ...
    if (s1InHOClusterChart) {
      s1InHOClusterChart.data.labels = band;
      s1InHOClusterChart.data.datasets[0].data = count;
      s1InHOClusterChart.data.datasets[0].backgroundColor = backgroundColors;
      s1InHOClusterChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !s1InHOClusterChart) {
      Chart.register(ChartDataLabels);

      s1InHOClusterChart = new Chart(ctx, {
