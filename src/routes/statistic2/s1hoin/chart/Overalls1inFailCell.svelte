<script lang="ts">
  ...
  function updateChart() {
    ...
    if (s1InHOCellChart) {
      s1InHOCellChart.data.labels = band;
      s1InHOCellChart.data.datasets[0].data = count;
      s1InHOCellChart.data.datasets[0].backgroundColor = backgroundColors;
      s1InHOCellChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !s1InHOCellChart) {
      Chart.register(ChartDataLabels);

      s1InHOCellChart = new Chart(ctx, {
