<script lang="ts">
  ...
  function updateChart() {
    ...
    if (s1InHOSiteChart) {
      s1InHOSiteChart.data.labels = band;
      s1InHOSiteChart.data.datasets[0].data = count;
      s1InHOSiteChart.data.datasets[0].backgroundColor = backgroundColors;
      s1InHOSiteChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !s1InHOSiteChart) {
      Chart.register(ChartDataLabels);

      s1InHOSiteChart = new Chart(ctx, {
