<script lang="ts">
  ...
  let s1InHOOverallChartByBand: Chart;
  ...
  function updateChart() {
    const OverallChartData = OverallData.Totals1InHONominate_byband.by_band;
    ...
    if (s1InHOOverallChartByBand) {
      s1InHOOverallChartByBand.data.labels = band;
      s1InHOOverallChartByBand.data.datasets[0].data = ratio;
      s1InHOOverallChartByBand.data.datasets[0].backgroundColor = backgroundColors;
      s1InHOOverallChartByBand.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !s1InHOOverallChartByBand) {
      Chart.register(ChartDataLabels);

      s1InHOOverallChartByBand = new Chart(ctx, {
