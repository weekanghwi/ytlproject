<script lang="ts">
  ...
  let VoLTEOverallFailCellChart: Chart;
  ...
  function updateChart() {
    const RRCSetupDatabyBand = OverallData.fail_cell_count_byband;
    ...
    if (VoLTEOverallFailCellChart) {
      VoLTEOverallFailCellChart.data.labels = band;
      VoLTEOverallFailCellChart.data.datasets[0].data = count;
      VoLTEOverallFailCellChart.data.datasets[0].backgroundColor = backgroundColors;
      VoLTEOverallFailCellChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !VoLTEOverallFailCellChart) {
      Chart.register(ChartDataLabels);

      VoLTEOverallFailCellChart = new Chart(ctx, {
