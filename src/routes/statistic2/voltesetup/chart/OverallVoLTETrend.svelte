<script lang="ts">
  ...
  let VoLTEConnectionOverallTrendChart: Chart;
  ...
  function updateChart() {
    const TrendData = OverallData.voltesetup_trend_all;
    if (!TrendData) return;
    const { weeknums, values } = TrendData;

    if (VoLTEConnectionOverallTrendChart) {
      VoLTEConnectionOverallTrendChart.data.labels = weeknums;
      VoLTEConnectionOverallTrendChart.data.datasets[0].data = values;
      VoLTEConnectionOverallTrendChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !VoLTEConnectionOverallTrendChart) {
      Chart.register(ChartDataLabels);

      VoLTEConnectionOverallTrendChart = new Chart(ctx, {
