<script lang="ts">
  ...
  let VoLTEOverallFailSiteChart: Chart;
  ...
  function updateChart() {
    ...
    if (VoLTEOverallFailSiteChart) {
      VoLTEOverallFailSiteChart.data.labels = band;
      VoLTEOverallFailSiteChart.data.datasets[0].data = count;
      VoLTEOverallFailSiteChart.data.datasets[0].backgroundColor = backgroundColors;
      VoLTEOverallFailSiteChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !VoLTEOverallFailSiteChart) {
      Chart.register(ChartDataLabels);

      VoLTEOverallFailSiteChart = new Chart(ctx, {
