<script lang="ts">
  ...
  export let VoLTESetupData: any = null; // 부모 컴포넌트에서 전달받은 데이터

  let canvas: HTMLCanvasElement;
  let VoLTEConnectionRegionalTrendChartbyBand: Chart;
  ...
  function updateChart() {
    const RegionalTrendData = VoLTESetupData.regional_erabsetup_trend_23;
    ...
    const datasets = [
      VoLTESetupData.regional_erabsetup_trend_23,
      VoLTESetupData.regional_erabsetup_trend_26,
      VoLTESetupData.regional_erabsetup_trend_800,
    ].filter(Boolean);
    ...
    if (VoLTEConnectionRegionalTrendChartbyBand) {
      VoLTEConnectionRegionalTrendChartbyBand.options.scales.y = {
        display: true,
        beginAtZero: false,
        min: yAxisMin,
        // max: yAxisMax,
      ...
      VoLTEConnectionRegionalTrendChartbyBand.data.labels = weeknums;
      VoLTEConnectionRegionalTrendChartbyBand.data.datasets[0] = {
        label: "2.3G Trend",
        data: values,
      ...
      const secondLineData = VoLTESetupData.regional_erabsetup_trend_26; // 
      const secondDataset = {
        label: "2.6G Trend",
      ...
      const thirdLineData = VoLTESetupData.regional_erabsetup_trend_800;
      const thirdDataset = {
        label: "800M Trend",
      ...
      // 데이터셋 배열 업데이트
      VoLTEConnectionRegionalTrendChartbyBand.data.datasets = [VoLTEConnectionRegionalTrendChartbyBand.data.datasets[0], secondDataset, thirdDataset];

      // 차트 업데이트
      VoLTEConnectionRegionalTrendChartbyBand.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !VoLTEConnectionRegionalTrendChartbyBand) {
      Chart.register(ChartDataLabels);

      VoLTEConnectionRegionalTrendChartbyBand = new Chart(ctx, {
