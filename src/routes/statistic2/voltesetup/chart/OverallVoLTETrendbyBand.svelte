<script lang="ts">
  ...
  let VoLTEConnectionOverallTrendChartbyBand: Chart;
  ...
  function updateChart() {
    const TrendData = OverallData.voltesetup_trend_23;
    if (!TrendData) return;
    ...
    const datasets = [
      OverallData.voltesetup_trend_23,
      OverallData.voltesetup_trend_26,
      OverallData.voltesetup_trend_800,
    ].filter(Boolean);
    ...
    if (VoLTEConnectionOverallTrendChartbyBand) {
      VoLTEConnectionOverallTrendChartbyBand.options.scales.y = {
        display: true,
        beginAtZero: false,
        min: yAxisMin,
        // max: yAxisMax,
      ...
      VoLTEConnectionOverallTrendChartbyBand.data.labels = weeknums;
      VoLTEConnectionOverallTrendChartbyBand.data.datasets[0] = {
        label: "2.3G Trend",
        data: values,
      ...
      const secondLineData = OverallData.voltesetup_trend_26; // 
      const secondDataset = {
        label: "2.6G Trend",
      ...
      const thirdLineData = OverallData.voltesetup_trend_800;
      const thirdDataset = {
        label: "800M Trend",
      ...
      // 데이터셋 배열 업데이트
      VoLTEConnectionOverallTrendChartbyBand.data.datasets = [VoLTEConnectionOverallTrendChartbyBand.data.datasets[0], secondDataset, thirdDataset];

      // 차트 업데이트
      VoLTEConnectionOverallTrendChartbyBand.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !VoLTEConnectionOverallTrendChartbyBand) {
      Chart.register(ChartDataLabels);

      VoLTEConnectionOverallTrendChartbyBand = new Chart(ctx, {
