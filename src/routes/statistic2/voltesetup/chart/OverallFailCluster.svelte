<script lang="ts">
  ...
  let VoLTEOverallFailClusterChart: Chart;
  ...
  function updateChart() {
    const RRCSetupDatabyBand = OverallData.fail_cluster_count_byband;
    ...
    if (VoLTEOverallFailClusterChart) {
      VoLTEOverallFailClusterChart.data.labels = band;
      VoLTEOverallFailClusterChart.data.datasets[0].data = count;
      VoLTEOverallFailClusterChart.data.datasets[0].backgroundColor = backgroundColors;
      VoLTEOverallFailClusterChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !VoLTEOverallFailClusterChart) {
      Chart.register(ChartDataLabels);

      VoLTEOverallFailClusterChart = new Chart(ctx, {
