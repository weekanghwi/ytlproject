<script lang="ts">
  ...
  export let CallDropnGBRData: any = null; // 부모 컴포넌트에서 전달받은 데이터

  let canvas: HTMLCanvasElement;
  let CallDropRegionalTrendChartbyBand: Chart;
  ...
  function updateChart() {
    const RegionalTrendData = CallDropnGBRData.regional_calldrop_trend_23;
    ...
    const datasets = [
      CallDropnGBRData.regional_calldrop_trend_23,
      CallDropnGBRData.regional_calldrop_trend_26,
      CallDropnGBRData.regional_calldrop_trend_800,
    ].filter(Boolean);
    ...
    if (CallDropRegionalTrendChartbyBand) {
      CallDropRegionalTrendChartbyBand.options.scales.y = {
        display: true,
        beginAtZero: false,
        min: yAxisMin,
        // max: yAxisMax,
      ...
      CallDropRegionalTrendChartbyBand.data.labels = weeknums;
      CallDropRegionalTrendChartbyBand.data.datasets[0] = {
        label: "2.3G Trend",
        data: values,
      ...
      const secondLineData = CallDropnGBRData.regional_calldrop_trend_26; // 
      const secondDataset = {
        label: "2.6G Trend",
      ...
      const thirdLineData = CallDropnGBRData.regional_calldrop_trend_800;
      const thirdDataset = {
        label: "800M Trend",
      ...
      // 데이터셋 배열 업데이트
      CallDropRegionalTrendChartbyBand.data.datasets = [CallDropRegionalTrendChartbyBand.data.datasets[0], secondDataset, thirdDataset];

      // 차트 업데이트
      CallDropRegionalTrendChartbyBand.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !CallDropRegionalTrendChartbyBand) {
      Chart.register(ChartDataLabels);

      CallDropRegionalTrendChartbyBand = new Chart(ctx, {
