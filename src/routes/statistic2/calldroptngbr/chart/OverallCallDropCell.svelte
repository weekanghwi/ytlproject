<script lang="ts">
  ...
  function updateChart() {
    const CallDropDatabyBand = OverallData.fail_cell_count_byband;
    ...
    if (CallDropCellChart) {
      CallDropCellChart.data.labels = band;
      CallDropCellChart.data.datasets[0].data = count;
      CallDropCellChart.data.datasets[0].backgroundColor = backgroundColors;
      CallDropCellChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !CallDropCellChart) {
      Chart.register(ChartDataLabels);

      CallDropCellChart = new Chart(ctx, {
