<script lang="ts">
  ...
  let CallDropOverallTrendChartbyBand: Chart;
  ...
  function updateChart() {
    const TrendData = OverallData.calldrop_trend_23;
    if (!TrendData) return;
    ...
    const datasets = [
      OverallData.calldrop_trend_23,
      OverallData.calldrop_trend_26,
      OverallData.calldrop_trend_800,
    ].filter(Boolean);
    ...
    if (CallDropOverallTrendChartbyBand) {
      CallDropOverallTrendChartbyBand.options.scales.y = {
        display: true,
        beginAtZero: false,
        min: yAxisMin,
        // max: yAxisMax,
      ...
      CallDropOverallTrendChartbyBand.data.labels = weeknums;
      CallDropOverallTrendChartbyBand.data.datasets[0] = {
        label: "2.3G Trend",
        data: values,
      ...
      const secondLineData = OverallData.calldrop_trend_26; // 
      const secondDataset = {
        label: "2.6G Trend",
      ...
      const thirdLineData = OverallData.calldrop_trend_800;
      const thirdDataset = {
        label: "800M Trend",
      ...
      // 데이터셋 배열 업데이트
      CallDropOverallTrendChartbyBand.data.datasets = [CallDropOverallTrendChartbyBand.data.datasets[0], secondDataset, thirdDataset];

      // 차트 업데이트
      CallDropOverallTrendChartbyBand.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !CallDropOverallTrendChartbyBand) {
      Chart.register(ChartDataLabels);

      CallDropOverallTrendChartbyBand = new Chart(ctx, {
