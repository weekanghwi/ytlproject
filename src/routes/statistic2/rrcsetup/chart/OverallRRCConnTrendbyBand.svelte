<script lang="ts">
  ...
  let RRCConnectionOverallTrendChartbyBand: Chart;
  ...
  function updateChart() {
    const TrendData = OverallData.rrcsetup_trend_23;
    if (!TrendData) return;
    ...
    const datasets = [
      OverallData.rrcsetup_trend_23,
      OverallData.rrcsetup_trend_26,
      OverallData.rrcsetup_trend_800,
    ].filter(Boolean);
    ...
    if (RRCConnectionOverallTrendChartbyBand) {
      RRCConnectionOverallTrendChartbyBand.options.scales.y = {
        display: true,
        beginAtZero: false,
        min: yAxisMin,
        // max: yAxisMax,
      ...
      RRCConnectionOverallTrendChartbyBand.data.labels = weeknums;
      RRCConnectionOverallTrendChartbyBand.data.datasets[0] = {
        label: "2.3G Trend",
        data: values,
      ...
      const secondLineData = OverallData.rrcsetup_trend_26; // 
      const secondDataset = {
        label: "2.6G Trend",
      ...
      const thirdLineData = OverallData.rrcsetup_trend_800;
      const thirdDataset = {
        label: "800M Trend",
      ...
      // 데이터셋 배열 업데이트
      RRCConnectionOverallTrendChartbyBand.data.datasets = [RRCConnectionOverallTrendChartbyBand.data.datasets[0], secondDataset, thirdDataset];

      // 차트 업데이트
      RRCConnectionOverallTrendChartbyBand.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !RRCConnectionOverallTrendChartbyBand) {
      Chart.register(ChartDataLabels);

      RRCConnectionOverallTrendChartbyBand = new Chart(ctx, {
