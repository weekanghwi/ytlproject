<script lang="ts">
  ...
  let RRCConnectionOverallTrendChart: Chart;
  ...
  function updateChart() {
    const TrendData = OverallData.rrcsetup_trend_all;
    if (!TrendData) return;
    const { weeknums, values } = TrendData;

    if (RRCConnectionOverallTrendChart) {
      RRCConnectionOverallTrendChart.data.labels = weeknums;
      RRCConnectionOverallTrendChart.data.datasets[0].data = values;
      RRCConnectionOverallTrendChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !RRCConnectionOverallTrendChart) {
      Chart.register(ChartDataLabels);

      RRCConnectionOverallTrendChart = new Chart(ctx, {
