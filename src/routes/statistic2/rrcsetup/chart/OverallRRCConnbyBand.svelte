<script lang="ts">
  ...
  let RRCConnectionOverallChartByBand: Chart;
  ...
  function updateChart() {
    const RRCSetupDatabyBand = OverallData.rrcattemptcoutbyband.by_band;
    if (!RRCSetupDatabyBand) return;
    const {band, values} = RRCSetupDatabyBand
    ...
    if (RRCConnectionOverallChartByBand) {
      RRCConnectionOverallChartByBand.data.labels = band;
      RRCConnectionOverallChartByBand.data.datasets[0].data = values;
      RRCConnectionOverallChartByBand.data.datasets[0].backgroundColor = backgroundColors;
      RRCConnectionOverallChartByBand.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !RRCConnectionOverallChartByBand) {
      Chart.register(ChartDataLabels);

      RRCConnectionOverallChartByBand = new Chart(ctx, {
