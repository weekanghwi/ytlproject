<script lang="ts">
  ...
  export let RRCSetupData: any = null; // 부모 컴포넌트에서 전달받은 데이터

  let canvas: HTMLCanvasElement;
  let RegionalRRCSetupTrendbyBandChart: Chart;
  ...
  function updateChart() {
    const TrendData = RRCSetupData.regional_rrcsetup_trend_23;
    if (!TrendData) return;
    ...
    const datasets = [
      RRCSetupData.regional_rrcsetup_trend_23,
      RRCSetupData.regional_rrcsetup_trend_26,
      RRCSetupData.regional_rrcsetup_trend_800,
    ].filter(Boolean);
    ...
    if (RegionalRRCSetupTrendbyBandChart) {
      RegionalRRCSetupTrendbyBandChart.options.scales.y = {
        display: true,
        beginAtZero: false,
        min: yAxisMin,
        // max: yAxisMax,
      ...
      RegionalRRCSetupTrendbyBandChart.data.labels = weeknums;
      RegionalRRCSetupTrendbyBandChart.data.datasets[0] = {
        label: "2.3G Trend",
        data: values,
      ...
      const secondLineData = RRCSetupData.regional_rrcsetup_trend_26; // 
      const secondDataset = {
        label: "2.6G Trend",
      ...
      const thirdLineData = RRCSetupData.regional_rrcsetup_trend_800;
      const thirdDataset = {
        label: "800M Trend",
      ...
      // 데이터셋 배열 업데이트
      RegionalRRCSetupTrendbyBandChart.data.datasets = [RegionalRRCSetupTrendbyBandChart.data.datasets[0], secondDataset, thirdDataset];

      // 차트 업데이트
      RegionalRRCSetupTrendbyBandChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !RegionalRRCSetupTrendbyBandChart) {
      Chart.register(ChartDataLabels);

      RegionalRRCSetupTrendbyBandChart = new Chart(ctx, {
