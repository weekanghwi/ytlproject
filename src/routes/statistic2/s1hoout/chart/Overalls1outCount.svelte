<script lang="ts">
  ...
  let s1OutHOOverallChartByBand: Chart;
  ...
  function updateChart() {
    const OverallChartData = OverallData.Totals1OutHONominate_byband.by_band;
    ...
    if (s1OutHOOverallChartByBand) {
      s1OutHOOverallChartByBand.data.labels = band;
      s1OutHOOverallChartByBand.data.datasets[0].data = ratio;
      s1OutHOOverallChartByBand.data.datasets[0].backgroundColor = backgroundColors;
      s1OutHOOverallChartByBand.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !s1OutHOOverallChartByBand) {
      Chart.register(ChartDataLabels);

      s1OutHOOverallChartByBand = new Chart(ctx, {
