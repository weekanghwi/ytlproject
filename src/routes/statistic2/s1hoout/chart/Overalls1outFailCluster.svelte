<script lang="ts">
  ...
  function updateChart() {
    ...
    if (s1OutHOClusterChart) {
      s1OutHOClusterChart.data.labels = band;
      s1OutHOClusterChart.data.datasets[0].data = count;
      s1OutHOClusterChart.data.datasets[0].backgroundColor = backgroundColors;
      s1OutHOClusterChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !s1OutHOClusterChart) {
      Chart.register(ChartDataLabels);

      s1OutHOClusterChart = new Chart(ctx, {
