<script lang="ts">
  ...
  let s1OutHOOverallTrendChart: Chart;
  ...
  function updateChart() {
    const TrendData = OverallData.s1outho_trend_all;
    if (!TrendData) return;
    const { weeknums, values } = TrendData;

    if (s1OutHOOverallTrendChart) {
      s1OutHOOverallTrendChart.data.labels = weeknums;
      s1OutHOOverallTrendChart.data.datasets[0].data = values;
      s1OutHOOverallTrendChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !s1OutHOOverallTrendChart) {
      Chart.register(ChartDataLabels);

      s1OutHOOverallTrendChart = new Chart(ctx, {
