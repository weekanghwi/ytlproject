<script lang="ts">
  ...
  function updateChart() {
    ...
    if (s1OutHOSiteChart) {
      s1OutHOSiteChart.data.labels = band;
      s1OutHOSiteChart.data.datasets[0].data = count;
      s1OutHOSiteChart.data.datasets[0].backgroundColor = backgroundColors;
      s1OutHOSiteChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !s1OutHOSiteChart) {
      Chart.register(ChartDataLabels);

      s1OutHOSiteChart = new Chart(ctx, {
