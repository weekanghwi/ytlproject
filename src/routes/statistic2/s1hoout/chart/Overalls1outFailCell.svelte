<script lang="ts">
  ...
  function updateChart() {
    ...
    if (s1OutHOCellChart) {
      s1OutHOCellChart.data.labels = band;
      s1OutHOCellChart.data.datasets[0].data = count;
      s1OutHOCellChart.data.datasets[0].backgroundColor = backgroundColors;
      s1OutHOCellChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !s1OutHOCellChart) {
      Chart.register(ChartDataLabels);

      s1OutHOCellChart = new Chart(ctx, {
