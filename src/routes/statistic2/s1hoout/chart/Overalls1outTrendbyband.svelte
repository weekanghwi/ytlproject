<script lang="ts">
  ...
  let s1OutHOOverallTrendChartbyBand: Chart;
  ...
  function updateChart() {
    const TrendData = OverallData.s1outho_trend_23;
    if (!TrendData) return;
    ...
    const datasets = [
      OverallData.s1outho_trend_23,
      OverallData.s1outho_trend_26,
      OverallData.s1outho_trend_800,
    ].filter(Boolean);
    ...
    if (s1OutHOOverallTrendChartbyBand) {
      s1OutHOOverallTrendChartbyBand.options.scales.y = {
        display: true,
        beginAtZero: false,
        min: yAxisMin,
        // max: yAxisMax,
      ...
      s1OutHOOverallTrendChartbyBand.data.labels = weeknums;
      s1OutHOOverallTrendChartbyBand.data.datasets[0] = {
        label: "2.3G Trend",
        data: values,
      ...
      const secondLineData = OverallData.s1outho_trend_26; // 
      const secondDataset = {
        label: "2.6G Trend",
      ...
      const thirdLineData = OverallData.s1outho_trend_800;
      const thirdDataset = {
        label: "800M Trend",
      ...
      // 데이터셋 배열 업데이트
      s1OutHOOverallTrendChartbyBand.data.datasets = [s1OutHOOverallTrendChartbyBand.data.datasets[0], secondDataset, thirdDataset];

      // 차트 업데이트
      s1OutHOOverallTrendChartbyBand.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !s1OutHOOverallTrendChartbyBand) {
      Chart.register(ChartDataLabels);

      s1OutHOOverallTrendChartbyBand = new Chart(ctx, {
