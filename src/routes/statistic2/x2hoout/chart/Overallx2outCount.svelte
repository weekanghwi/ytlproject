<script lang="ts">
  ...
  let x2OutHOOverallChartByBand: Chart;
  ...
  function updateChart() {
    const OverallChartData = OverallData.Totalx2OutHONominate_byband.by_band;
    ...
    if (x2OutHOOverallChartByBand) {
      x2OutHOOverallChartByBand.data.labels = band;
      x2OutHOOverallChartByBand.data.datasets[0].data = ratio;
      x2OutHOOverallChartByBand.data.datasets[0].backgroundColor = backgroundColors;
      x2OutHOOverallChartByBand.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !x2OutHOOverallChartByBand) {
      Chart.register(ChartDataLabels);

      x2OutHOOverallChartByBand = new Chart(ctx, {
