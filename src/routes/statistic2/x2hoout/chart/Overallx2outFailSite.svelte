<script lang="ts">
  ...
  function updateChart() {
    ...
    if (x2OutHOSiteChart) {
      x2OutHOSiteChart.data.labels = band;
      x2OutHOSiteChart.data.datasets[0].data = count;
      x2OutHOSiteChart.data.datasets[0].backgroundColor = backgroundColors;
      x2OutHOSiteChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !x2OutHOSiteChart) {
      Chart.register(ChartDataLabels);

      x2OutHOSiteChart = new Chart(ctx, {
