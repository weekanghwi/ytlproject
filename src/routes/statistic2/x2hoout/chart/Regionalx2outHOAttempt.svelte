<script lang="ts">
  ...
  export let x2OutHORegionalData: any = null;
  export let chartKey: string;

  let canvas: HTMLCanvasElement;
  let x2OutHOAttemptChart: Chart;
  ...
  function updateChart(chartKey: string) {
    const RegionalData = processAPIData(x2OutHORegionalData, chartKey)
    ...
    if (x2OutHOAttemptChart) {
      x2OutHOAttemptChart.data.labels = band;
      x2OutHOAttemptChart.data.datasets[0].data = ratio;
      x2OutHOAttemptChart.data.datasets[0].backgroundColor = backgroundColors;
      x2OutHOAttemptChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !x2OutHOAttemptChart) {
      Chart.register(ChartDataLabels);

      x2OutHOAttemptChart = new Chart(ctx, {
