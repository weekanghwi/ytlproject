<script lang="ts">
  ...
  function updateChart() {
    ...
    if (x2OutHOClusterChart) {
      x2OutHOClusterChart.data.labels = band;
      x2OutHOClusterChart.data.datasets[0].data = count;
      x2OutHOClusterChart.data.datasets[0].backgroundColor = backgroundColors;
      x2OutHOClusterChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !x2OutHOClusterChart) {
      Chart.register(ChartDataLabels);

      x2OutHOClusterChart = new Chart(ctx, {
