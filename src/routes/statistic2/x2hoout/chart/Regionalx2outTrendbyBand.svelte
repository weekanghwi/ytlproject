<script lang="ts">
  ...
  export let x2OutHORegionalData: any = null; // 부모 컴포넌트에서 전달받은 데이터

  let canvas: HTMLCanvasElement;
  let x2OutHORegionalTrendChartbyBand: Chart;
  ...
  function updateChart() {
    const RegionalTrendData = x2OutHORegionalData.regional_x2outho_trend_23;
    ...
    const datasets = [
      x2OutHORegionalData.regional_x2outho_trend_23,
      x2OutHORegionalData.regional_x2outho_trend_26,
      x2OutHORegionalData.regional_x2outho_trend_800,
    ].filter(Boolean);
    ...
    if (x2OutHORegionalTrendChartbyBand) {
      x2OutHORegionalTrendChartbyBand.options.scales.y = {
        display: true,
        beginAtZero: false,
        min: yAxisMin,
        // max: yAxisMax,
      ...
      x2OutHORegionalTrendChartbyBand.data.labels = weeknums;
      x2OutHORegionalTrendChartbyBand.data.datasets[0] = {
        label: "2.3G Trend",
        data: values,
      ...
      const secondLineData = x2OutHORegionalData.regional_x2outho_trend_26; // 
      const secondDataset = {
        label: "2.6G Trend",
      ...
      const thirdLineData = x2OutHORegionalData.regional_x2outho_trend_800;
      const thirdDataset = {
        label: "800M Trend",
      ...
      // 데이터셋 배열 업데이트
      x2OutHORegionalTrendChartbyBand.data.datasets = [x2OutHORegionalTrendChartbyBand.data.datasets[0], secondDataset, thirdDataset];

      // 차트 업데이트
      x2OutHORegionalTrendChartbyBand.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !x2OutHORegionalTrendChartbyBand) {
      Chart.register(ChartDataLabels);

      x2OutHORegionalTrendChartbyBand = new Chart(ctx, {
