<script lang="ts">
  ...
  function updateChart() {
    ...
    if (x2OutHOCellChart) {
      x2OutHOCellChart.data.labels = band;
      x2OutHOCellChart.data.datasets[0].data = count;
      x2OutHOCellChart.data.datasets[0].backgroundColor = backgroundColors;
      x2OutHOCellChart.update();
    }
  }

  onMount(() => {
    const ctx = canvas.getContext("2d");
    if (ctx && !x2OutHOCellChart) {
      Chart.register(ChartDataLabels);

      x2OutHOCellChart = new Chart(ctx, {
