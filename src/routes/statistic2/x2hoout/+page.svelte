<script lang="ts">
  ...
  import Overallx2outCount from './chart/Overallx2outCount.svelte';
  // // import OverallInterFreqHoAttempt from './chart/OverallInterFreqHOAttempt.svelte';
  import Overallx2outTrend from './chart/Overallx2outTrend.svelte';
  import Overallx2outTrendbyband from './chart/Overallx2outTrendbyband.svelte';
  import Overallx2outFailCluster from './chart/Overallx2outFailCluster.svelte';
  import Overallx2outFailSite from './chart/Overallx2outFailSite.svelte';
  import Overallx2outFailCell from './chart/Overallx2outFailCell.svelte';
  ...
  onMount(async () => {
    try {
      OverallData = data.x2OutHOStatisticData.x2OutHO;
    } catch (error) {
  ...
  <div class="flex flex-col gap-2 mb-4">
    <div class="flex items-center gap-2">
      <p class="text-xs text-slate-400 ms-1">
        {data.x2OutHOStatisticData.x2OutHO.x2outho_trend_all.weeknums[9]} X2 OUT HANDOVER
      </p>
    </div>
  ...
  <div class="grid grid-cols-8 gap-4 mb-4">
    ...
    <div class="flex flex-col col-span-2 rounded-md bg-slate-900/40 p-4">
      <div class="flex items-center gap-1 mb-4">
        ...
        <h1 class="text-xs text-slate-500">X2 OUT HO ATTEMPT</h1>
      </div>
      <div class="flex items-center justify-around">
        <div class="relative flex items-center justify-center">
          <div class="z-50">
            <Overallx2outCount {OverallData} />
          </div>
          <div class="absolute flex flex-col items-center">
            <h1 class="text-xs text-slate-300">X2 OUT HO Attempt</h1>
            <h1 class="text-XS text-white font-bold">
              {(data.x2OutHOStatisticData.x2OutHO.Totalx2OutHONominate_byband.overall.Totalx2OutHONominate).toLocaleString()}
            </h1>
          </div>
        </div>
        ...
        <div class="flex flex-col justify-center gap-2">
          <div class="flex flex-col gap-1">
            {#each Object.keys(data.x2OutHOStatisticData.x2OutHO.Totalx2OutHONominate_byband.by_band.band) as index}
            <div class="flex items-center gap-1">
              <div class="h-3 w-3 rounded-sm 
              {data.x2OutHOStatisticData.x2OutHO.Totalx2OutHONominate_byband.by_band.band[index] === '800M' ? 'bg-sky-400' : 
              data.x2OutHOStatisticData.x2OutHO.Totalx2OutHONominate_byband.by_band.band[index] === '2.6GHz' ? 'bg-sky-600' : 'bg-sky-800'}"></div>
              <h1 class="text-xs text-white">{data.x2OutHOStatisticData.x2OutHO.Totalx2OutHONominate_byband.by_band.band[index]}</h1>
            </div>
            <h1 class="text-xs text-slate-500 ps-4">
              {(data.x2OutHOStatisticData.x2OutHO.Totalx2OutHONominate_byband.by_band.ratio[index]).toFixed(2)}%
            </h1>
            {/each}
          </div>
        </div>
    ...
    <div class="flex flex-col col-span-3 rounded-md bg-slate-900/40 p-4">
      <div class="flex items-center justify-between mb-4">
        <div class="flex items-center gap-1">
          <Icon icon="f7:number-square" class="text-lg text-slate-500" />
          <h1 class="text-xs text-slate-500">X2 OUT HO Nominate Items Count & Ratio</h1>
        </div>
    ...
    <div class="flex flex-col col-span-2 rounded-md bg-slate-900/40">
      <div class="relative flex items-center gap-1 p-4">
        ...
        <h1 class="text-xs text-slate-500">X2 OUT HO WEEKLY TREND</h1>
        <div class="absolute top-10 left-5 flex items-center">
          <h1 class="text-2xl text-white font-bold">
            {(data.x2OutHOStatisticData.x2OutHO.x2outho_trend_all.values[9]).toFixed(2)}%
          </h1>
          <p class="{`text-xs ps-2 ${(data.x2OutHOStatisticData.x2OutHO.x2outho_trend_all.values[9] - data.x2OutHOStatisticData.x2OutHO.x2outho_trend_all.values[8]) < 0 ? 'text-rose-400' : 'text-green-400' }`}">
            {(data.x2OutHOStatisticData.x2OutHO.x2outho_trend_all.values[9] - data.x2OutHOStatisticData.x2OutHO.x2outho_trend_all.values[8]).toFixed(2)}
          </p>
          {#if (data.x2OutHOStatisticData.x2OutHO.x2outho_trend_all.values[9] - data.x2OutHOStatisticData.x2OutHO.x2outho_trend_all.values[8]) > 0}
      ...
      <Overallx2outTrend {OverallData} />
    </div>

    <!-- X2 OUT HO TREND BY BAND -->
    <div class="flex flex-col col-span-3 rounded-md bg-slate-900/40">
      ...
      <Overallx2outTrendbyband {OverallData} />
    </div>

    <!-- X2 OUT HO CLUSTER, SITE AND CELL COUNT -->
    <div class="flex flex-col col-span-3 rounded-md bg-slate-900/40 p-4">
      <div class="flex items-center gap-1 mb-4">
      ...
      <div class="flex items-center justify-around gap-3">
        <div class="relative flex items-center justify-center">
          <div class="z-50">
            <Overallx2outFailCluster {OverallData} />
          </div>
          <div class="absolute flex flex-col items-center justify-center">
            <h1 class="text-xs text-slate-400">Fail cluster</h1>
            <h1 class="text-2xl text-white font-bold">
              {(data.x2OutHOStatisticData.x2OutHO.fail_cluster_count_all).toLocaleString()}
            </h1>
          </div>
        </div>
        ...
        <div class="relative flex items-center justify-center">
          <div class="z-50">
            <Overallx2outFailSite {OverallData} />
          </div>
          <div class="absolute flex flex-col items-center justify-center">
            <h1 class="text-xs text-slate-400">Fail cluster</h1>
            <h1 class="text-2xl text-white font-bold">
              {(data.x2OutHOStatisticData.x2OutHO.fail_site_count_all).toLocaleString()}
            </h1>
          </div>
        </div>
        ...
        <div class="relative flex items-center justify-center">
          <div class="z-50">
            <Overallx2outFailCell {OverallData} />
          </div>
          <div class="absolute flex flex-col items-center justify-center">
            <h1 class="text-xs text-slate-400">Fail cluster</h1>
            <h1 class="text-2xl text-white font-bold">
              {(data.x2OutHOStatisticData.x2OutHO.fail_cell_count_all).toLocaleString()}
            </h1>
          </div>
        </div>
