<script lang="ts">
  ...
  import { trendDataStore, fetchTrendData } from "./Fetchx2OutHOData";
	import { Tooltip } from "flowbite-svelte";


  import Regionalx2outHoAttempt from "../chart/Regionalx2outHOAttempt.svelte";
  import Regionalx2outTrendbyBand from "../chart/Regionalx2outTrendbyBand.svelte";
  import Regionalx2outFailCluster from "../chart/Regionalx2outFailCluster.svelte";
  import Regionalx2outFailSiteCell from "../chart/Regionalx2outFailSiteCell.svelte";
  ...
  let field: string = 'x2_handover_out_success_rate';
  let x2OutHORegionalData: any = null;
  let band: string;

  $: x2OutHORegionalData = $trendDataStore
  $: $trendDataStore, x2OutHORegionalData = $trendDataStore
  $: if (region) {
    (region);
  }
  ...
  const x2OutHOAttemptCountChartKeyMap: { [key: string]: string} = {
    'Totalx2OutHONominate_byband': 'Total Attempt',
  };
  const chartKeys = Object.keys(x2OutHOAttemptCountChartKeyMap);
  ...
  $: selectedNominateData = getSelectedData(x2OutHORegionalData, selectedChartKey)
  ...
  $: SelectedNominateOverallData = getSelectedNominateOverallData(x2OutHORegionalData, selectedChartKey)
...
<div class="grid grid-cols-8 gap-4 w-full mb-4">
  ...
  <div class="flex flex-col col-span-2 bg-slate-900/40 rounded-md p-4">
    <div class="flex items-center gap-1">
      ...
      <h1 class="text-xs text-slate-500">{region.toLocaleUpperCase()} X2 OUT HO ATTEMPT COUNT</h1>
    </div>

    {#if x2OutHORegionalData}
      <div class="flex flex-col items-center justify-center h-full">
        <div class="flex flex-col">
          <h1 class="text-2xl text-white font-bold">
            {((x2OutHORegionalData.Totalx2OutHONominate_byband.overall.Totalx2OutHONominate) - 
            (x2OutHORegionalData.Totalx2OutHONominate_byband.overall.Totalx2OutHOdeNominate)).toLocaleString()}
          </h1>
          <h1 class="text-xs text-slate-500">
            {(((x2OutHORegionalData.Totalx2OutHONominate_byband.overall.Totalx2OutHONominate) - 
              (x2OutHORegionalData.Totalx2OutHONominate_byband.overall.Totalx2OutHOdeNominate)) / 7).toLocaleString()} (Per Day)
          </h1>
        </div>
      </div>
      ...
      <div class="flex items-center gap-1">
        <div class="rounded-sm px-1 py-[1px] bg-green-500 text-xs">
          {
            ((((x2OutHORegionalData.Totalx2OutHONominate_byband.overall.Totalx2OutHONominate) - 
              (x2OutHORegionalData.Totalx2OutHONominate_byband.overall.Totalx2OutHOdeNominate)) / 
              ((data.x2OutHOStatisticData.x2OutHO.Totalx2OutHONominate_byband.overall.Totalx2OutHONominate) - 
              (data.x2OutHOStatisticData.x2OutHO.Totalx2OutHONominate_byband.overall.Totalx2OutHOdeNominate))) * 
              100).toFixed(1)
          }%
        </div>
        <h1 class="text-xs text-slate-500">Share of the national X2 out ho fail count</h1>
      </div>
    {/if}
  </div>

  <!-- REGIONAL X2 OUT HO COUNT BY BAND -->
  <div class="flex flex-col col-span-2 bg-slate-900/40 rounded-md p-4 gap-4">
    <div class="relative flex items-start gap-1">
      <Icon icon="fluent:calendar-week-numbers-24-regular" class="text-lg text-slate-500" />
      <div class="flex flex-col">
        <h1 class="text-xs text-slate-500">X2 OUT HO ATTEMPT BY BAND</h1>
      </div>
      ...
      {#if showDropdown}
        <div class="z-50 absolute -top-5 right-5 flex flex-col bg-slate-700/90 shadow-md rounded-md mt-2">
          ...
          on:click={() => {selectedChartKey = key; showDropdown = false}}>
            {x2OutHOAttemptCountChartKeyMap[key]}
          </button>
          {/each}
        </div>
      {/if}
    ...
    <div class="flex items-center gap-4">
      <div class="relative flex items-center justify-center">
        <div class="">
          <Regionalx2outHoAttempt {x2OutHORegionalData} chartKey={selectedChartKey} />
        </div>
        <div class="absolute flex flex-col items-center justify-center gap-1">
          ...
          <h1 class="text-xs text-white">{x2OutHOAttemptCountChartKeyMap[selectedChartKey]}</h1>
        </div>
      </div>
  ...
  <!-- REGIONAL X2 OUT HO TREND BY BAND -->
  <div class="flex flex-col col-span-4 bg-slate-900/40 rounded-md p-4">
    <div class="flex items-center gap-1 mb-4">
      <Icon icon="fluent:calendar-week-numbers-24-regular" class="text-lg text-slate-500" />
      <h1 class="text-xs text-slate-500">{region.toLocaleUpperCase()} X2 OUT HO TREND BY BAND</h1>
    </div>
    <div class="">
      <Regionalx2outTrendbyBand {x2OutHORegionalData} />
    </div>
  </div>
  ...
  <div class="flex flex-col col-span-2 bg-slate-900/40 rounded-md p-4">
    <div class="flex items-center gap-1 mb-4">
      <Icon icon="fluent:calendar-week-numbers-24-regular" class="text-lg text-slate-500" />
      <h1 class="text-xs text-slate-500">{region.toLocaleUpperCase()} X2 OUT HO FAIL CLUSTER</h1>
    </div>
    <div class="flex items-center justify-around">
      <div class="relative flex">
        <Regionalx2outFailCluster {x2OutHORegionalData} />
        <div class="absolute top-[35%] left-[28%] flex flex-col items-center justify-center">
          ...
          <h1 class="text-2xl text-white font-bold">
            {#if x2OutHORegionalData}{x2OutHORegionalData.fail_cluster_count_all}{/if}
          </h1>
        </div>
      </div>
      ...
      <div class="flex flex-col gap-2">
        <div class="flex flex-col">
          <div class="flex flex-col gap-4">
            {#if x2OutHORegionalData && x2OutHORegionalData[FailClusterCount]}
              {#each x2OutHORegionalData[FailClusterCount].band as bands, index}
                <div class="flex items-center">
                  <button class="flex items-center justify-between p-2 gap-2 text-xs text-white w-full rounded-md 
                    ...
                    <h1 class="text-xs text-black w-4 h-4 rounded-full bg-rose-300">
                      {x2OutHORegionalData[FailClusterCount].count[index]}
                    </h1>
                  </button>
                </div>
  ...
  <!-- REGIONAL X2 OUT HO FAIL SITE & CELL -->
  <div class="flex flex-col col-span-3 bg-slate-900/40 rounded-md p-4">
    <div class="flex items-center gap-1 mb-4">
      <Icon icon="fluent:calendar-week-numbers-24-regular" class="text-lg text-slate-500" />
      <h1 class="text-xs text-slate-500">{region.toLocaleUpperCase()} X2 OUT HO FAIL SITE & CELL</h1>
    </div>
    <div class="flex items-center justify-around">
      <div class="relative flex">
        <Regionalx2outFailSiteCell {x2OutHORegionalData} />
        <div class="absolute top-[35%] left-[25%] flex flex-col items-center justify-center">
          ...
          <h1 class="text-lg text-white font-bold">
            {#if x2OutHORegionalData}
              {x2OutHORegionalData.fail_site_count_all}/{x2OutHORegionalData.fail_cell_count_all}
            {/if}
          </h1>
        </div>
      </div>
      <div class="grid grid-cols-3 gap-2">
        ...
        <div class="flex items-center justify-center w-full px-1 py-[1px] rounded-sm bg-rose-400">
          <h1 class="text-xs text-white">
            {#if x2OutHORegionalData}
              {#if x2OutHORegionalData.fail_site_count_byband.band.includes('2.3GHz')}
                {x2OutHORegionalData.fail_site_count_byband.count[x2OutHORegionalData.fail_site_count_byband.band.indexOf('2.3GHz')]}
              {:else}
              -
              {/if}
            {/if}
        ...
        <div class="flex items-center justify-center w-full px-1 py-[1px] rounded-sm bg-rose-400">
          <h1 class="text-xs text-white">
            {#if x2OutHORegionalData}
              {#if x2OutHORegionalData.fail_cell_count_byband.band.includes('2.3GHz')}
                {x2OutHORegionalData.fail_cell_count_byband.count[x2OutHORegionalData.fail_site_count_byband.band.indexOf('2.3GHz')]}
              {:else}
              -
              {/if}
            {/if}
        ...
        <div class="flex items-center justify-center w-full px-1 py-[1px] rounded-sm bg-rose-400">
          <h1 class="text-xs text-white">
            {#if x2OutHORegionalData}
              {#if x2OutHORegionalData.fail_site_count_byband.band.includes('2.6GHz')}
                {x2OutHORegionalData.fail_site_count_byband.count[x2OutHORegionalData.fail_site_count_byband.band.indexOf('2.6GHz')]}
              {:else}
              -
              {/if}
            {/if}
        ...
        <div class="flex items-center justify-center w-full px-1 py-[1px] rounded-sm bg-rose-400">
          <h1 class="text-xs text-white">
            {#if x2OutHORegionalData}
              {#if x2OutHORegionalData.fail_cell_count_byband.band.includes('2.6GHz')}
                {x2OutHORegionalData.fail_cell_count_byband.count[x2OutHORegionalData.fail_site_count_byband.band.indexOf('2.6GHz')]}
              {:else}
              -
              {/if}
            {/if}
        ...
        <div class="flex items-center justify-center w-full px-1 py-[1px] rounded-sm bg-rose-400">
          <h1 class="text-xs text-white">
            {#if x2OutHORegionalData}
              {#if x2OutHORegionalData.fail_site_count_byband.band.includes('800M')}
                {x2OutHORegionalData.fail_site_count_byband.count[x2OutHORegionalData.fail_site_count_byband.band.indexOf('800M')]}
              {:else}
              -
              {/if}
            {/if}
        ...
        <div class="flex items-center justify-center w-full px-1 py-[1px] rounded-sm bg-rose-400">
          <h1 class="text-xs text-white">
            {#if x2OutHORegionalData}
              {#if x2OutHORegionalData.fail_cell_count_byband.band.includes('800M')}
                {x2OutHORegionalData.fail_cell_count_byband.count[x2OutHORegionalData.fail_site_count_byband.band.indexOf('800M')]}
              {:else}
              -
              {/if}
            {/if}
  ...
  <!-- REGIONAL X2 OUT HO MOST FAIL CLUSTER AND CELL LIST -->
  <div class="flex flex-col col-span-3 bg-slate-900/40 rounded-md">
    ...
    <div class="flex items-center rounded-md pb-2">
      <table class="w-full">
        ...
        <tbody>
          {#if x2OutHORegionalData && x2OutHORegionalData[topImpactClusterListKey]}
            {#each Object.entries(x2OutHORegionalData[topImpactClusterListKey]) as [band, data]}
              {#each data.cluster as cluster, index}
                <tr class="text-xs text-slate-300 py-2">
                  <td class="py-1 px-4">{cluster}</td>
                  <td class="py-1 px-4">{(data.x2outhofailtotal[index]).toLocaleString()}</td>
                  <td class="py-1 px-4">{((data.x2outhofailtotal[index] / x2OutHORegionalData.x2outhofallbybandall.by_band.failcount[2]) * 100).toFixed(2)}%</td>
  ...
  <div class="flex flex-col col-span-3 bg-slate-900/40 rounded-md">
    <div class="flex items-center p-4 gap-1">
      ...
      <h1 class="text-xs text-slate-500">X2 OUT HO CLUSTER</h1>
    </div>
  ...
  <div class="flex flex-col col-span-5 bg-slate-900/40 rounded-md">
    <div class="flex items-center p-4 gap-1">
      ...
      <h1 class="text-xs text-slate-500">X2 OUT HO CELL</h1>
    </div>
