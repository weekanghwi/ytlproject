<script lang="ts">
  ...
  //Btsmanager export
  const exportBTSmanager = () => {
    window.location.href = 'http://10.24.8.120:8000/api/export_btsmanager/'
  }
...
<div class="p-2 w-full pt-2">
  ...
  <Breadcrumb aria-label="siteinfo breadcrumb" class="mb-6">
    ...
    <BreadcrumbItem>YTL LTE Site List</BreadcrumbItem>
  </Breadcrumb>

  <!-- Table Title -->
  <div class="flex items-center gap-4 mb-3">
    <h5 class="text-2xl text-gray-800 dark:text-gray-300">Base site information</h5>
    <button on:click={exportBTSmanager} class="text-xs py-1 px-2 rounded-md bg-lime-400 hover:bg-lime-500 focus:ring-4 focus:ring-lime-600">Export BTS Manager to CSV</button>
  </div>
  ...
  <Table shadow={true} hoverable={true} customeColor={'bg-gray-400'} class="w-full">
    <TableHead>
      ...
      <TableHeadCell class="py-4 text-sm">Detail</TableHeadCell>
    </TableHead>
      ...
      {#each data.sitelists.data.results as sites}
        <TableBodyRow color="custom" class="dark:bg-gray-700/30 border-b border-gray-500/50">
          ...
          <TableBodyCell class="py-2">
            <span class="dark:text-gray-400 truncate w-[160px] inline-block">
              {sites.cluster}
            </span>
          </TableBodyCell>
