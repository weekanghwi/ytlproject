<script lang="ts">
  ...
  function updateChart() {
    const bandData = SitecountChartData.sitecount_by_regionband['38'];
    console.log(bandData)
    if (!bandData) return;
    const { region, count } = bandData;

    const regionColors: Record<string, string> = {
      Central: 'rgba(186,230,253, 0.8)',
      Northern: 'rgba(125,211,252, 0.8)',
      Southern: 'rgba(56,189,248, 0.8)',
      Eastern: 'rgba(14,165,233, 0.8)',
      Sabah: 'rgba(2,132,199, 0.8)',
      Sarawak: 'rgba(3,105,161, 0.8)'
    }
