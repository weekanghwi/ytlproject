<script lang="ts">
  ...
  import { fetchOPTReviewDataone, updateOPTReviewData } from './crud'
  ...
  export let updateOPTReviewModal = false;
  ...
  $: if (reviewId) {
    (async () => {
      try {
        const data = await fetchOPTReviewDataone(reviewId);
        optreviewData = data;
      } catch (error) {
  ...
  async function handleSubmit(event: SubmitEvent) {
    ...
    try {
      ...
      await updateOPTReviewData(reviewId, optreviewData);
      updateOPTReviewModal = false;
      errors = {};
      // dispatch('doUpdated');
...
<Modal bind:open={updateOPTReviewModal} size="xs" autoclose={false} class="w-full">
  <form class="flex flex-col w-full" action="#" on:submit|preventDefault={handleSubmit}>
    <div class="flex items-center gap-2 mb-6">
      <h3 class="text-md font-medium text-gray-900 dark:text-white">Update OPT Review</h3>
    ...
    <div class="flex flex-col gap-3 mb-4 pb-4">
      {optreviewData.sitebasicinfo} {optreviewData.reviewdate} Review data
    </div>
    ...
    <div class="flex gap-4 mt-5">
      <Button color="green" type="submit" class="w-fit py-1.5 px-3">Update OPT Review</Button>
      <Button color='red' on:click={() => {updateOPTReviewModal = false}} class="w-fit py-1.5 px-3">Cancel</Button>
    </div>
    </form>
</Modal>